*! boottest 1.5.0 7 September 2016
...
void boottestModel::boottest() {
	real colvector rAll, numer_i, _e, eUZVR0wt, ID1, Ystar, beta, betaEnd
	real rowvector val, YstarYstar
	real matrix betadevEx, betadevEnd, betanumer, RAll, L, LAll, vec, denom, combs, XExZVR0wt, XEndZVR0wt, ZVR0wt, t, ZExclYstar, XExYstar, Subscripts, Zi, AVR0, betadenom, eZVR0, SeuZVR0, SeZVR0, eu
	real scalar i, j, c
	pointer (real matrix) scalar _pR0, pewt, pSeZVR0, pXEndstar, pXExXEndstar, pZExclXEndstar, pu
	...
	if (WREnonAR) {
		...
		for (j=cols(u); j; j--) { // WRE bootstrap
			...
			if (robust) { // Compute denominator for this WRE test stat
				...
				for (c=1; c<=length(clust); c++) {
					if (c==1 & Nclust) {
						AVR0 = pM_Repl->A * pM_Repl->VR0; beta = -pM_Repl->beta \ 1; betaEnd = beta[|kEx+1\.|]

						SeuZVR0 = (beta'XZi[clust.N].M + betaEnd'eZi[clust.N].M * u[clust.N,j]) * AVR0 // R0 * V * Z_i'estar_i
						denom = cross(SeuZVR0, SeuZVR0)
						for (i=clust.N-1; i; i--) {
							SeuZVR0 = (beta'XZi[i].M + betaEnd'eZi[i].M * u[i,j]) * AVR0 // R0 * V * Z_i'estar_i
		...
		if (scoreBS)
			...
			betadenom = K? (*pM->pV * pM->A ') : *pM->pV // in IV, this is actually not denominator (V) but V * X'Z(Z'Z)^-1
		...
		else if (!null) numer[,1] = *pR0 * pM->beta - *pr0 // Analytical Wald numerator; if imposing null then numer[,1] already equals this. If not, then it's 0 before this.
		...
		if (robust) {
			...
			if (df == 1) {  // optimize for one null constraint
				Dist = sqrt? numer / sqrt(*pR0 * pM->VR0) : (numer:*numer) / (*pR0 * pM->VR0)
				if (ML)
					Dist = Dist'
				else {
					             eu = u :* pM->e
...
real vector _selectindex(real vector v) {
	real colvector i, t; real matrix w; real vector _v
	pragma unset i; pragma unset w
	_v = v :!= 0
	maxindex((cols(v)==1? _v \ 0 : _v, 0), 1, i, w)
...
void boottestModel::plot(real scalar level) {
	...
	if (gridstart==. | gridstop==.) {
		if (reps)
			if (AR) {
				t = abs(cuepoint) * (small? invt(df_r, alpha/2)/invt(df_r, get_padj(1)/2) : invnormal(alpha/2)/invnormal(get_padj(1)/2))
			...
			t = abs(numer[1]/Dist[1]) * (small? invt(df_r, alpha/2) : invnormal(alpha/2))
...
// return matrix whose rows are all the subsets of a row of numbers. Nil is at top.
