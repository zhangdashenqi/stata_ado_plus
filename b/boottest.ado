...
program define _boottest, rclass sortpreserve
	...
	if !`ML' {
		...
		cap mata `C' = st_matrix("`C'" )[,(st_matrix("`keepC'"),`=`k'+1')]; `C' = `C'[_selectindex(rowsum(`C':!=0)),]; st_matrix("`C'" , `C')
		if `GMM' st_matrix("`W'", st_matrix("`W'" )[st_matrix("`keepW'"), st_matrix("`keepW'")])
	...
	if `hasclust' {
		if 0`override' {
			cap assert !missing(`:subinstr local clustvars " " ",", all') if e(sample), fast
			if _rc {
	...
	forvalues h=1/`N_h0s' { // loop over multiple independent constraints
		...
		if `ML' {
			...
			if `null' {
				...
				if !`rc' {
					tempname b0
					mat `b0' = e(b)
					cap `anything' if e(sample), `=cond(inlist("`cmd'", "cmp","ml"),"init","from")'(`b0') iterate(0) `options'
					local rc = _rc
				}
				if `rc' {
			...
			mata `C' = st_matrix("`C0'")[,(st_matrix("`keepC'"),`=`k'+1')]; `C' = `C'[_selectindex(rowsum(`C':!=0)),]; st_matrix("`C0'", `C')
		}
		...
		if "`graph'"=="" & "`plotmat'"!="" {
			...
			line `Y' `X', sort(`X') || scatter `Y' `X' if _n>rowsof(`plotmat'), mlabel(`X') mlabpos(6) xtitle("") ///
				ytitle(`"`=strproper("`madjust'")+ cond("`madjust'"!="","-adjusted r", "R")'ejection p value"') ///
				yscale(range(0 .)) name(`graphname'`_h', `replace') ///
				`=cond(`level'<100, "yline(`=1-`level'/100') ylabel(#6 `=1-`level'/100')", "")' legend(off) `graphopt'
