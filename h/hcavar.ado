...
version 9
syntax varlist(min=2 numeric) [,PROX(string) METHod(string) PARTition(numlist) MEASures  DETect MATrix(string)]
...
if "`type'">"2"&"`prox'"!="pearson"  {
   di in red "Only the {hi:pearson} measure of proximity is available with ordinal or numerous variables"
   ...
   forvalues i=1/`nbitems' {
	forvalues j=`=`i'+1'/`nbitems' {	
		...
		/***********************************Proximity CCOR**********************/
			...
			matrix `proximity'[`i',`j']=sqrt(2*(1-`dij'/(`N'-`nnull')))
			matrix `proximity'[`j',`i']=`proximity'[`i',`j']
		}
...
local hor "hor"
if "`method'"!="centroid"&"`method'"!="median" {
