...
{
/*
  Estimate percentile slopes and their confidence limitsts for censlope.
...
real scalar pctcur, beta, betaleft, betaright, betafarleft, betafarright,
  ...
  pctlseq, tdist, df_r, eform, xmin, xmax, ymin, ymax,
...
if(!missing(bracketmat)){
  for(pctlseq=1;pctlseq<=rows(cimat);pctlseq++){
    ...
    */
    ...
    _somdtransf(transf , 1 :- 2 :* pctcur :/ 100 , zetacur)
...
*/
real scalar subrc
string scalar yvar, xvar, touse, ystar
...
*/
subrc=_stata("somersd " + xvar + " "+ystar+" if " + touse + ", " + st_local("somdopts"),1)
if(subrc!=0){
  error(subrc)
  return(2)
}
