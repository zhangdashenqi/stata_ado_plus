...
program define _cmp, sortpreserve properties(user_score svyb svyj svyr mi fvaddcons) byable(recall)
	version 10.0
	cap version 11.0
	...
	if $cmp_d != `:word count `:subinstr local t ")" "", all'' cmp_error 198 `"The indicators() option must contain $cmp_d variables, one for each equation. Did you forget to type {stata "cmp setup"}?"'
	...
	qui {
		...
		while `"`1'"' != "" {
			...
			if _rc {
				...
				label values ${cmp_y`cmp_eqno'} . // modifying user's data labels!
				cap egen int _cmp_y`cmp_eqno' = group(${cmp_y`cmp_eqno'}) if ${cmp_y`cmp_eqno'} < ., lname(cmp_y`cmp_eqno'_label) // trap errors to prevent crash while user's data modified
				if _rc {
					local rc = _rc
				...
				label values ${cmp_y`cmp_eqno'} `label'
				sum _cmp_y`cmp_eqno' if `touse' & _cmp_ind`cmp_eqno'==$cmp_oprobit, meanonly
				local t = r(max) - 1
			...
			if `N_mprobit' | `N_roprobit' { // multinomial or rank-ordered probit
				...
				if `asprobit_eq'==1 & "`m_ro'" == "" { // starting new asprobit group?
					...
					mat cmp_`m_ro'probit_group_inds = nullmat(cmp_`m_ro'probit_group_inds) \ (`cmp_eqno', .)
				}
				...
				if `asprobit_eq' == 0 { // non-as mprobit?
					...
					egen int _cmp_y`cmp_eqno' = group(${cmp_y`cmp_eqno'}) if `touse' & _cmp_ind`cmp_eqno'==$cmp_mprobit, lname(cmp_y`cmp_eqno'_label)

					sum _cmp_y`cmp_eqno', meanonly
					local num_alts = r(max) - 1
					if `num_alts' == 0 cmp_error 148 "There is only one outcome in ${cmp_y`cmp_eqno'}."
					...
					replace _cmp_ind`cmp_eqno' = $cmp_mprobit_ind_base + _cmp_y`cmp_eqno' + `cmp_eqno' - 1 if `touse' & _cmp_ind`cmp_eqno'==$cmp_mprobit
			...
			if _rc==0 {
				global cmp_y`cmp_eqno' .
				qui replace _cmp_ind`cmp_eqno' = . if `touse'
			}
	...
	forvalues l = 1/$parse_L {
		forvalues j=1/$cmp_d {
			...
			global cmp_rceq`l' ${cmp_rceq`l'} `="${cmp_eq`j'} " * `: word count ${cmp_rc`l'_`j'} ${cmp_re`l'_`j'}''
		}
	}
...
// /atanhrho_[coef1]_[coef2]_[lev]_[eq1][eq2] across, upper levels, REs not on both sides
	...
	`quietly' DoInitSearch InitSearch if `touse' `wgtexp', `svy' adjustprobitsample `drop' auxparams(`auxparams') `=cond("`init'" == "", "", "quietly")' mlopts(`mlopts')
	mat `cmpInitFull' = r(b)
	...
	if "`meff'" != "" {
		...
		qui InitSearch if `touse' & (`subpop'), `drop' auxparams(`auxparams') mlopts(`mlopts')
		mat `b' = r(b)
		Estimate `method_spec' if `touse' & (`subpop'), init(`init') cmpinit(`b') `vce' auxparams(`auxparams') psampling(`psampling') resteps(`steps') bounds(`bounds') ///
			constraints(`constraints' `_constraints' `initconstraints') `autoconstrain' `mlopts' `technique' `quietly' redraws(`redraws') paramsdisplay(`r(ParamsDisplay)')
		if _rc==0 {
			tempname vsmp
			mat `vsmp' = e(V)
		}
	...
	if `"`lrtest'`constraints'`robust'`cluster'`estimate'`svy'"'=="" & "`weight'"!="pweight" & "$parse_x"!="" {
		...
		qui InitSearch if `touse' `wgtexp', `svy' 1only  auxparams(`auxparams') mlopts(`mlopts')
		local 1onlyinitconstraints `r(initconstraints)'
		mat `b' = r(b)
		qui Estimate `method_spec' if `touse' `wgtexp', cmpinit(`b') constraints(`_constraints' `1onlyinitconstraints') `autoconstrain' psampling(`psampling') resteps(`steps') ///
		                `svy' `mlopts' `technique' auxparams(`r(auxparams)') 1only `quietly' redraws(`redraws') paramsdisplay(`r(ParamsDisplay)') bounds(`bounds')
	...
	cmp_full_model `method_spec' if `touse' `wgtexp', `vce' score(`score') `lf0opt' modopts(`modopts') mlopts(`mlopts') `technique'  paramsdisplay(`ParamsDisplay') xvarsall(`XVarsAll') ///
		constraints(`_constraints' `initconstraints' `constraints') init(`init') cmpinit(`cmpInitFull') `svy' `interactive' psampling(`psampling') ///
...
program Estimate, eclass
	version 10.0
	cap version 11.0
	syntax anything(name=method_spec) if/ [fw aw pw iw], [auxparams(string) psampling(string) svy score(string) interactive autoconstrain paramsdisplay(string) bounds(string) ///
		modopts(string) mlopts(string) init(string) cmpinit(string) constraints(string) technique(string) 1only quietly lnl(string) resteps(string) redraws(string) *]
...
/*	if "`constraints'" != "" {
		tempname b V Cns CnsCoef CnsCoefCols a C
		mat `b' = `cmpinit'
		mat `V' = `b'' * `b'
		ereturn post `b' `V'
		makecns `constraints'
		if e(Cns) == "" local Cns // if all constraints bad, no e(Cns) made
		else {
			mat `Cns' = e(Cns)
	...
	if $cmpHasGamma {
		...
		if "`constraints'" != "" { // as necessary, translate constraints expressed in terms of displayed names into terms of /gamma_X_Y
			tempname b V T a C Cns
			...
			local i 0
			forvalues eq=1/$cmp_d { // makeCns doesn't like names suffixed with #; rename them temporarily
				tempname v`++i'
				foreach c of numlist `_constraints' {
			...
			ereturn post `b' `V' `C'
			mat `b' = e(b)
			mat colnames `b' = `:colfullnames `_init''
			eret repost b = `b', rename
			mat `Cns' = e(Cns)
			constraint drop `_constraints'
			local constraints `Cns'
		}
	}
	forvalues eq=1/$cmp_d { 
			...
			foreach macro in y x xo xe {
				if "${cmp_`macro'`eq'}" != "" & ("`macro'"!="x" | "`1only'"=="") {
					if "`macro'${cmp_y`eq'}"=="y." local y_revar _cmp_ind1 // ml doesn't like "." for a depvar
						...
						if c(stata_version)>=11 { // stick "o." back on fvrevar'd omitted variables
							tokenize `r(varlist)'
							fvexpand ${cmp_`macro'`eq'} if _cmp_ind`eq'
							local `macro'_revar
							forvalues i=1/`:word count `r(varlist)'' {
								local `macro'_revar ``macro'_revar' `=cond(strpos("`:word `i' of `r(varlist)''", "o."), "o.", "")'``i''  
							}
						}
						else local `macro'_revar `r(varlist)'
					}
				}
				else local `macro'_revar
			}
			forvalues l=1/$parse_L {
				if "${cmp_rc`l'_`eq'}" != "" {
...
*			local x_revar_all `x_revar_all' `x_revar'
		}
		if "`1only'"=="" local model `model' $cmp_gammaparams
		local model `method_spec' `model' `auxparams'
/*		if "`Cns'" != "" {
			mata _colnames = st_matrixcolstripe("`Cns'")
			mata _t = tokens("`x_revar_all'")
			mata _colnames[|1,2 \ length(_t),2|] = _t'
			mata st_matrixcolstripe("`Cns'", _colnames)
		}
*/
	}
			...
			forvalues restep = 1/`resteps' {
				...
				if "`interactive'" == "" {
					...
					local mlcmd `quietly' version `mlversion': ml model `model' `=cond(`final',"[`weight'`exp'], `options' score(`score')", "`awgtexp',")' max ///
						`svy' constraints(`constraints' /*`Cns'*/) nocnsnotes nopreserve missing collinear `this_mlopts' `modopts' bounds(`bounds') technique(`this_technique')
					...
					if "`constraints'" == "" | $cmpHasGamma mat colnames `b' = `:colnames `cmpinit''
					...
					local mlmodelcmd version `mlversion': ml model `model' `=cond(`final',"[`weight'`exp'] if `if',`options'","`awgtexp' if (`if') & `u'<=.001+`psampling_cutoff', ")' ///
						`svy' constraints(`Cns') nocnsnotes `modopts' collinear technique(`this_technique') nopreserve missing
...
program InitSearch, rclass
	...
	forvalues eq=1/$cmp_d {
		...
		if colsof(`beta') {
			cap mat `betavec' = nullmat(`betavec'), `beta'
			if _rc {
	...
	if $cmpHasGamma {
		local ParamsDisplay `:colfullnames `betavec'' `:colfullnames `gammavec'' 
	...
	forvalues l=1/$parse_L {
		...
		quietly forvalues eq=1/$cmp_d {
			if cmp_nonbase_cases[1,`eq'] {
				...
				if `l' < $parse_L {
					tempname ebar`eq'
					by `cmp_ids': egen `ebar`eq'' = mean(`e`eq'')
...
program Display, eclass
	...
	if e(k_gamma) | e(k_gamma_reducedform)<. {
		if `"`resultsform'"' != "" {
			...
			if "`form'" != "`curform'" {
				tempname b V Cns b`curform' V`curform' hold
