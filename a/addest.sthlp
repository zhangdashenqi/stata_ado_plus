{smcl}
{* *! Help file version 1.3 written by Mead Over (mover@cgdev.org) 9Oct2015}{...}
...
stored in ereturn by allowing the user to add items to those stored 
...
the estimation results of several models in a similar table include {help xml_tab}
and {help outreg}.  
...
The {cmdab:n:ame} option specifies the name of the scalar to be added to the ereturn space.
...
numbers of rows and columns as those currently in the ereturn space.
...
the father's height, {cmd:fheight}, the mother's height {cmd:mheight} and on the child's age, 
...
{p 4 4 2}
All three dummies would be zero if the child is less than or equal 5 years old.
...
{p 4 4 2}
The user-written table-making commands {help outreg} and {help xml_tab} 
...
{p 4 8 2} {help ereturn}, if installed: {help xml_tab}, if installed: {help outreg}  
