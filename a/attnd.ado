...
tempvar ycnew sameps newweight mfweight mbweight foweight bweight fpsdif bpsdif fbmatt bbmatt fyc byc fw bw pscoref pscoreb fdiff bdiff tweight stweight pick
...
qui g `pick'=uniform() /* generates random numbers between [0,1) */

/* in case of equally good forward and backward matches randomly pick one direction and set the respective fdiff or bdiff to 100 which is by far larger than any possible difference in pscoreS */

qui replace `fdiff'=100 if `fdiff'==`bdiff' & `T'==1 & `pick'>=0.5 
qui replace `bdiff'=100 if `fdiff'==`bdiff' & `T'==1 & `pick'<0.5
...
by `PSCORE' `T': egen `mbweight' = max (`bweight') 
...
/* foweight==0 if a control is never a forward best match
   ...
   => only for controls that are both forward and backward best 
   matches tweight is the sum of two positive weights */
...
qui replace PSDIF =abs(`bpsdif') if abs(`bpsdif') == abs(`fpsdif') & `T'==1 & `pick'<0.5
qui replace PSDIF =abs(`fpsdif') if abs(`bpsdif') == abs(`fpsdif') & `T'==1 & `pick'>=0.5
...
return scalar attnd   = `myt'-`myc'
return scalar seattnd = sqrt( `varyt'/`nttot' + (`nweight'/(`nttot'^2))*`varyc' )
return scalar tsattnd = return(attnd) / return(seattnd)
return scalar ntnd = `nttot'
return scalar ncnd = `nc'


di _newline(3) _column(1)  in ye "ATT estimation with Nearest Neighbor Matching method " 
di in ye       _column(1)  "(random draw version)" 
...
*/             _column(38)  in gr "Std. Err." /*
*/             _column(49)  in gr "        t"
di in gr             in text "{hline 57}"
di _newline(1) _column(1)   in ye %9.0f return(ntnd) /*
*/             _column(13)  in ye %9.0f return(ncnd) /*
*/             _column(25)  in ye %9.3f return(attnd) /*
*/             _column(38)  in ye %9.3f return(seattnd) /*
*/             _column(49)  in ye %9.3f return(tsattnd)
di in gr _newline(1) in text "{hline 57}"
...
bs "attnd `varlist' `if' `in', pscore(`pscore') `logit' `index' `comsup' " attnd=r(attnd), nowarn `options'
...
return scalar attnd = _b[attnd]
return scalar bseattnd = _se[attnd]
return scalar btsattnd = return(attnd) / return(bseattnd)

di _newline(3) _column(1)  in ye "ATT estimation with Nearest Neighbor Matching method" 
di             _column(1)  "(random draw version)" 
...
*/             _column(13) in ye %9.0f return(ncnd) /*
*/             _column(25) in ye %9.3f return(attnd) /*
*/	     _column(37) in ye %9.3f return(bseattnd) /*
*/	     _column(49) in ye %9.3f return(btsattnd)
di in gr _newline(1) in text "{hline 57}"
di in gr "Note: the numbers of treated and controls refer to actual"
di in gr "nearest neighbour matches"
...
qui attnd `varlist' `if' `in' [`weight'`exp'], pscore(`pscore') `logit' `index' `comsup'
...
if `"`detail'"' != `""'  {  
  di in ye _newline (3) "***************************************************************************** "
  di in ye             "End of the estimation with the nearest neighbor matching (random draw) method "
  di in ye             "***************************************************************************** "
}
