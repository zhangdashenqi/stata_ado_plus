*! version 2.1.4 1feb2007  Richard Williams, rwilliam@nd.edu
...
// Replay results if that is all that is requested. 
	...
	// you that gologit2 generated the results even if e(cmd) = gologit.
	// This makes it possible to replay results when v1 has been specified.

	if replay() {
		if "`e(cmd)'" != "gologit2" & "`e(cmd2)'" != "gologit2"  {
...
// Additional checks on the varlist and pl & npl specfications
	...
	if "`npl2'"!="" {
		...
		if "`nplchek'"!="" {
			...
			display as error "The -force- option lets you proceed anyway"
			display ""
			`comment' exit 198
		}
	}
...
// Generate all the new things we will need for the models
	...
	if `M' > 20 {
		...
		di as error "The -force- option lets you proceed anyway"
		di
		`comment'exit 149
	}
...
// Fit the mis-specified model if meff has been requested - svy only
	if "`meff'" != ""  {
		...
		quietly ml model lf gologit2_ll `eqs' if `subtouse', ///
			constraints(`constraints')   ///
...
// Final cleanup
	...
	// may not work correctly with gologit2.  Specifying v1 can zap programs
	// designed to work with gologit2.
	// e(cmd2) indicates that gologit2 generated the results rather than gologit.
	if "`v1'"=="v1" {
		v1_format
		ereturn local cmd2 "gologit2"
...
// the calculation of Pseudo R^2 is wrong.
	...
	if "`mlstart'"!="" {
		if "`svy'"=="" {
			quietly ml model lf gologit2_ll `eqsx' `wgt' if `subtouse', 		///
				`robust'  `clopt'  	///
				waldtest(-`Numeqs')  	///
...
// LL0 will contain the log likelihood 0 for non-svy models
	...
	// = sum over j of N * Pj * ln(Pj)
		...
		if "`svy'"=="" {
			quietly proportion `y' `wgt' if `subtouse' , `clopt'
			mat `b0' = e(b)
			forval j = 1/`M' {
			...
			quietly svy, `subpop_options': proportion `y' if `touse'
			mat `b0' = e(b)
		}
