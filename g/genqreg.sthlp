...
{pstd}
{opt scale(#)} Experience indicates that sometimes moment conditions can be poorly-scaled for MCMC sampling in a way that
may depend upon the situation. By specifying a scalar using {cmd:scale(#)}, the user can "tighten" the parameter 
distribution implied by moment conditions. That is, if the value of the moment condition is
{it:-V }, employing the option {scale(2)} would replace the moment condition with {it:-2V}.
