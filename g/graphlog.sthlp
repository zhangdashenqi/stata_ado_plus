...
{hi:help graphlog}{right: ({browse "http://www.stata-journal.com/article.html?article=up0055":SJ17-2: gr0064_1})}
{hline}
...
{phang}
{cmd:color} specifies the text color in the PDF.  The default is {cmd:color(black)}.
For a (noncomplete) list of available colors, go to
{browse "https://en.wikibooks.org/wiki/LaTeX/Colors#The_68_standard_colors_known_to_dvips"}

{phang}
{cmd:encoding} specifies the character encoding of the log file.  The default
is {cmd:encoding(uft8)} on Linux/Mac OS X and {cmd:encoding(ansinew)} on
Windows.  If {cmd:graphlog} does not understand your non-English characters,
try another character encoding as described on
{browse "https://en.wikibooks.org/wiki/LaTeX/Special_Characters#Input_encoding"}
...
{cmd:pdflatex} program used by {cmd:graphlog}. To fix this, type

{phang2}
{bf:{stata "graphlog_unix_setup":graphlog_unix_setup}}
...
{pstd}
{cmd:graphlog} generally supports paths and filenames containing blanks but not
leading blanks.  That is, {cmd:graphlog} understands when a graph is stored as
"{bind:my example .pdf}" or the directory is "{bind:C:/Random user/Documents }",
but it does not understand the filename "{bind: example.pdf}" or the
directory "{bind:C:/ RandomUser/Documents}".
...
{phang2}{it:({stata "graphlog_example":click to run})}{p_end}
...
{p 4 14 2}Article:  {it:Stata Journal}, volume 17, number 2: {browse "http://www.stata-journal.com/article.html?article=up0055":gr0064_1},{break}
                    {it:Stata Journal}, volume 15, number 2: {browse "http://www.stata-journal.com/article.html?article=gr0064":gr0064}{p_end}
