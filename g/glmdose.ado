...
#delimit ;
glmgpscore `varlist' `if' `in'  [`weight' `exp'], family(`family') link(`link')  
t(`t')
gpscore(`gpscore') 
nq_gps(`nq_gps') 
predict(`predict') 
...
if ("`npoints'"=="" & "`tpoints'"=="") {
sort `t'
qui duplicates report  `t' if `touse'
local ntreat_values = r(unique_value) 
qui duplicates tag `t' if `touse', gen(`tag')
sort `t'
qui gen `id_treat'=.
qui replace  `id_treat' = 1 if `tag'==0
qui replace  `id_treat' = 1 if `tag'>0 & `t'[_n-1]!=`t'[_n] & `tag'<.
...
matrix def `tvector'= `tvector' \ `mat_aux'
local i = 	`i' + 1
}
}
...
#delimit ;
qui glmgpscore `varlist' `if' `in', family(`family') link(`link')  
t(`t')
