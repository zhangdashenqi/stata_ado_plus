{smcl}
{hline}
{cmd:help: {helpb gs3sls}}{space 55} {cmd:dialog:} {bf:{dialog gs3sls}}
{hline}

{bf:{err:{dlgtab:Title}}}

{bf:gs3sls: Generalized Spatial Autoregressive Three Stage Least Squares (3SLS)}
...
{p 5}{helpb gs3sls##03:Model Options}{p_end}
...
{p 5 5 6} 
{opt mfx(lin, log)} {opt ord:er(#)} {opt coll zero tolog} {opt nocons:tant}{p_end} 
{p 5 5 6} 
...
{marker 02}{bf:{err:{dlgtab:Description}}}
 ...
 {cmd:gs3sls} estimates Generalized Spatial Autoregressive
...
{col 4}{cmd:e(rss)}{col 20}Residual SS
...
{col 4}{cmd:e(mfx)}{col 20}Beta, Total, Direct, and InDirect Marginal Effect
{col 4}{cmd:e(mfxe)}{col 20}Beta, Total, Direct, and InDirect Elasticity 
...
{col 4}{cmd:e(ilog)}{col 20}iteration log
...
{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(1) mfx(lin)}

{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(1) mfx(lin)}

{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(2) aux(x5)}

{bf:* (2) Generalized Spatial 3SLS - AR(2) (gs3sls)}
{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(2)}

{bf:* (3) Generalized Spatial 3SLS - AR(3) (gs3sls)}
{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(3)}

{bf:* (4) Generalized Spatial 3SLS - AR(4) (gs3sls)}
{stata gs3sls y1 x1 x2 , var2(y2 x3 x4) wmfile(SPWcs) eq(1) order(4)}
{hline}

{bf:{err:* Generalized Spatial Autoregressive 3SLS (GS3SLS)} (Cont.)}
* This example is taken from Prucha data about:
* Estimation of Simultaneous Systems of Spatially Interrelated Cross Sectional Equations
* More details can be found in:
* {browse "http://econweb.umd.edu/~prucha/Research_Prog4.htm"}
* Results of {bf:({err:{it:gs3sls}})} with order(2) is identical to:
* {browse "http://econweb.umd.edu/~prucha/STATPROG/SIMEQU/PROGRAM4.log"}

	{stata clear all}
	{stata sysuse gs3sls1.dta , clear}
	{stata gs3sls y1 x1 , var2(y2 x2) wmfile(SPWcs1) order(2)}
{hline}
...
- Schwartz Criterion            SC                =   370.0454
...
{p 1 10 1}{cmd:GS3SLS: "Generalized Spatial Autoregressive Three Stage Least Squares Cross Sections Regression"}{p_end}
