*! version 1.0.4  07aug2007  Ben Jann
...
program gsample
    ...
    syntax [anything(name=n)] [if] [in] [aw/] [, ///
...
*/	
    ...
    if `"`strata'`cluster'"'=="" & `"`generate'"'=="" {
        _gsample1 `n' `"`nvar'"' `N' `Nout' `touse' `"`exp'"' ///
         `"`generate'"' `"`wor'"' `"`percent'"' `"`keep'"' `"`replace'"'
        exit
    }
    if `"`strata'`cluster'"'=="" {
        _gsample1g `n' `"`nvar'"' `N' `Nout' `touse' `"`exp'"' ///
         `"`generate'"' `"`wor'"' `"`percent'"' `"`keep'"' `"`replace'"'
        exit
    }
    if `"`generate'"'=="" {
        _gsample2 `n' `"`nvar'"' `N' `Nout' `touse' `"`exp'"' ///
         `"`generate'"' `"`wor'"' `"`percent'"' `"`keep'"' `"`replace'"' ///
         `"`strata'"' `"`cluster'"' `"`idcluster'"'
        exit
    }
    _gsample2g `n' `"`nvar'"' `N' `Nout' `touse' `"`exp'"' ///
     `"`generate'"' `"`wor'"' `"`percent'"' `"`keep'"' `"`replace'"' ///
     `"`strata'"' `"`cluster'"' `"`idcluster'"'
end

program _gsample1 // no strata, no clusters, return sample
    args n nvar N Nout touse exp generate wor percent keep replace
    if `"`nvar'"'!="" {
