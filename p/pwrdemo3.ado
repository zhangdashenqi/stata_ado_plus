*! up to version 8, 1/24/04
*! revised 6/20/01
capture program drop pwrdemo3
program define pwrdemo3
  version 6.0
...
program define _mkgr
  quietly {
    ...
    local b1 = "note( Power = " + string(power) + ", pos(1) ring(0))"
    version 8.2: twoway (line y1 x) (line y2 x) (line y3 x, recast(area) pstyle(p2) bfcolor(erose)), ///
                 xlabel(-1.0 -.5 to 1)  xscale(range(-1.2 1.2)) ylabel(0) legend(off) ///
                 subtitle("`t1'") caption("`t2'", pos(11) ring(0)) `b1' `options'
...
program define _dbox
  ...
  * adjust N
  global DB_nmin  "_chn -5"
  global DB_nplus "_chn  5"
  window control button "N - 5"             5  10 35 10 DB_nmin
  window control button "N + 5"             45 10 35 10 DB_nplus

  * adjust diff
  global DB_dmin  "_chd -.05"
  global DB_dplus "_chd  .05"
  window control button "diff - .05"         5  25 35 10 DB_dmin
  window control button "diff + .05"         45 25 35 10 DB_dplus
  ...
  * window control scombo DB_slpL     55 105 25 40 DB_slp parse(,)
  ...
  window control button "Done"      5 60 75 10 DB_done
  window dialog "Power Demo" . . 90 90
end
