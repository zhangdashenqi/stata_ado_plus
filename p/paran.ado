*! version 1.4.3 12sep2011 by alexis dot dinno at pdx dot edu
*! perform horn's parallel analysis of principle components/factors

*   Copyright Notice
*   paran and paran.ado are Copyright (c) 2001, 2011 alexis dinno
*
...
* Syntax:  paran varlist [weight] [if exp] [in range] [, iterations(#) centile(#) quietly nostatus nostatus status factor(factor_type) citerate(#) protect(#) pnf all graph color lcolors(# # # # # # # # #) saving(file) replace seed(#) copyleft]
...
program define paran8, rclass
 version 8.0
 syntax varlist(numeric) [aweight fweight] [if] [in] [, ITErations(integer 0) CENTile(numlist min=1 max=1 >0 <100) Quietly NOSTatus STatus factor(string) CITerate(passthru) PRotect(passthru) pnf all GRaph color lcolors(numlist integer missingok min=9 max=9 >=0 <=255) saving(string) replace seed(integer 0) copyleft]
 preserve
 quietly {
...
* display the copyleft information if requested
  ...
  if "`copyleft'" == "copyleft" {
    noisily {
      ...
      di "paran and paran.ado are Copyright (c) 2001, 2011 alexis dinno" _newline
