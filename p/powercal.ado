...
if `"`tdf'"'=="" {;
  * Normal distribution *;
  if "`result'"!="power" {;
    tempvar invfb;
    qui {;
      gene double `invfb'=invnorm(`power_v') if `touse';
      compress `invfb';
    };
  };
  if "`result'"!="alpha" {;
    tempvar invfa;
    qui {;
      gene double `invfa'=-invnorm(0.5*`alpha_v') if `touse';
      compress `invfa';
    };
  };
};
...
if "`result'"=="power" {;
  qui{;
    ...
    if `"`tdf'"'=="" {;replace `tempres'=norm(`tempres') if `touse';};
  ...
  qui{;
    ...
    if `"`tdf'"'=="" {;
      replace `tempres'=norm(`tempres') if `touse';
    };
    else{;
