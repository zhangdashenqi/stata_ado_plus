*! version 1.1 Stephen P. Jenkins, Aug 1998     STB-48 sg104
...
program define povdeco
	...
	tempvar wi touse fi poor fgt0 fgt1 fgt2 badinc /*
         */ nk vk fik meanyk meanypk mngapk fgt0k fgt1k fgt2k /*
	...
	*************************
	...
	egen `meanypk' = sum(`fik'*`poor'*`inc')/`fgt0k'[1] /*
		*/ if `touse', by(`bygroup')

	egen `mngapk' = sum(`fik'*(`z'-`inc')*`poor')/`fgt0k'[1]  /*
		*/  if `touse', by(`bygroup')
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
		*/ & `touse', c(`vk' `meanyk' `meanypk' `mngapk') f(%9.5f)

	noi di "              "
	noi di "Subgroup FGT index estimates, FGT(a)"
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
		*/ & `touse', c(`fgt0k' `fgt1k' `fgt2k') f(%9.5f)

	drop `nk' `meanyk' `meanypk' `mngapk'
	...
	* NB. Sk(0) = (#poor in k)/(total #poor); i.e. 'composition'
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
		*/ & `touse', c(`share0k' `share1k' `share2k') f(%9.5f)
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
		*/ & `touse', c(`risk0k' `risk1k' `risk2k') f(%9.5f)
