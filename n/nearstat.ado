...
*! 1.0.3 June 2009: 
*! Statistics can now be calculated for rings, nearest neighbors, or all neighbors
...
program define nearstat, rclass
	...
	syntax varlist(min=2 max=2 numeric) [if] [in], near(varlist min=2 max=2 numeric) DISTvar(str) ///
		 ...
		 EXPDist(str) expto(str) SPArse(str) NOZero des(str) replace favor(str) iidist(str)]
	mata: mata clear
	...
	* Checking the coordinates
	...
	if "`cart'"=="" {
		if `lat1'>90 | `lat1'<-90 | `lat2'>90 | `lat2'<-90 | `long1'>180 | `long1'<-180 | `long2'>180 | `long2'<-180 {
			di
			di as err ///
			" Latitudes/Longitudes invalid: latitudes must be in [-90,90] and longitudes must be in [-180,180]"
      		exit 198
		} 
...
** Now create report
	...
	di
	di in y " Distance `forunit' calculation completed successfully and/or all requests processed" 
	if "`cart'"!="" {
		di
