...
{p 2 8 2}
{bf:nearstat --- Calculates distance-based variables and exports distance matrix to text file.}
...
{p 2}{help nearstat##acknow:Acknowledgments}{p_end}
...
{phang}
{cmdab: nearstat} {varlist:1}{cmd:,} {cmd:near({varlist:2})} {cmdab:dist:var(}{newvar:1}{cmd:)} [ {it:Other options} ] 
...
{synopt :{opth statv:ar(newvarlist)}}specify a list of variable names to hold the descriptive statistics calculated over nearest neighbors{p_end}
...
{synopt :{opt knn(#>=2)}}specify the number of nearest neighbors to be used{p_end}

{synopt :{opth db:and(numlist)}}indicate a distance cut-off (or a ring, or a distance band){p_end}

{synopt :{opt all:nei}}use all neighbors{p_end}

{synopt :{cmdab:dn:ame(}{newvar:3}{cmd:)}}request a dummy variable equal to 1 if the specified nearest neighbors are within {opt dband()}{p_end}

{synopt :{cmdab:nc:ount(}{newvar:4}{cmd:)}}request a variable holding the number of neighbors falling in ring {opt dband()}{p_end}

{synopt :{cmdab:inc:dist(}{newvar:5}{cmd:)}}request a variable holding incremental distance to reach a metropolitan area population threshold{p_end}
...
{synopt :{cmdab:iidist(}{newvar:6}{cmd:)}}request a variable holding one-to-one distance between input and near features{p_end}
...
{synopt :{cmd:des(}{it:des_option,}[{it:des_suboption}]{cmd:)}}display descriptive statistics for distance between input and near features, 
	   distance to the kth-nearest neighbors, and neighbor count within a distance band{p_end}
...
{pstd}
{cmd:nearstat} generates distance-based variables by calculating, for each observation in a Stata dataset, distance to  
nearest neighbors and descriptive statistics over those neighbors and exports the distances to a Stata matrix, a Mata file, or a text file.
Distance is calculated as the Great Circle or crow-fly distance depending on whether spherical or Cartesian coordinates are supplied 
to {cmd:nearstat}. 
...
{pstd}
Shapefiles for counties, census tracts or metropolitan areas are available from the U.S. Census Bureau's website (see the references below). 
One way to obtain Cartesian or projected coordinates is to project the shapefiles in ArcGIS using appropriate projected coordinate systems. 
More importantly, spherical coordinates (latitudes and longitudes in decimal degrees) for most geographic areas (counties, census tracts, 
metropolitan areas) in the U.S can be downloaded with any of the 2000 Census Summary files.
...
{phang}
{cmd:near({varlist:2})} specifies the variables holding latitudes and longitudes of the observations or areal units
to which distance needs to be calculated. This option is required. 

{pmore}{varlist:1} holds latitudes and longitudes of the observations or areal units from which distance needs to be calculated.
...
from each county to its nearest neighboring county or to obtain for each county the average per capita income of the eight nearest 
...
{phang}
{cmd:distvar(}{newvar:1}{cmd:)} specifies a variable name for holding distance from each input feature to its kth nearest neighbor. This option 
is required. 
...
{phang}
{cmd:nid(}{varname} {newvar:2}{cmd:)} requests the identity (observation number in the current sort order) or name of 
the nearest neighbors specified in {opt kth(#)}. This option requires two variable names. The first one is the identifier variable for the 
near features. The other is a variable name to hold the requested identities. Obviously, if {varlist:1} = {varlist:2} then there is only 
one identifier variable for both input and near features. If an observation has two or more equidistant neighbors, given the order considered, 
{cmd:nearstat} will report the first one encountered.
...
{phang}
{opth statvar(newvarlist)} provides a list of variable names to hold the calculated descritive statistics or to hold the values of the variables in 
{varlist:3} corresponding to the nearest neighbors. Specify one variable name for each variable in {varlist:3}.
...
{phang}
{opth dband(numlist)} indicates a distance band to be used with option {opt dname()} and/or {opt ncount()}, or requests that statistics be calculated 
for near features falling in the ring specified with {bf:dband()}. 

{pmore}{bf:Note 5:} When {opt dband()} is specified, by default, the distance unit is assumed to be kilometers, but that can be overriden
...
{phang}
{opt alpha(#)} requests distance weighted statistics. For example, if {opt alpha(1)} is specified, {cmd:nearstat} will divide the values of the variables
listed in {varlist:3} by distance prior to calculating the statistics. Specifying {opt alpha(2)} entails dividing by distance squared.

{phang}
{cmd:dname(}{newvar:3}{cmd:)} provides a variable name for holding a dummy variable equal to one if a nearest neighbor specified with {opt kth()} is within
the distance band specified with {opt dband()} and zero otherwise.

{phang}
{cmd:ncount(}{newvar:4}{cmd:)} specifies a variable name to hold, for each observation, the number of neighbors that are within the specified distance band.

{pmore}{bf:Note 6:} When {opt allnei} or {opt knn(#)} is specified, specifying {opt dband()} implies a request for a neighbor count or for a dummy variable. 
As a result, either {opt ncount()} or {opt dname()} must be specified.

{phang}
{cmd:incdist(}{newvar:5}{cmd:)} specifies a variable name to hold incremental distance to reach the threshold population specified with {opt atpop()} 
(see Partridge and Rickman, 2008). {opt incdist()} and {opt statvar()} may not be combined. 
...
{phang}
{cmd:iidist(}{newvar:6}{cmd:)} specifies a variable name to hold one-to-one distance between input and near features when they are different but
have the same number of non-missing observations. Essentially, this variable holds the diagonal elements of the distance matrix.

{phang}
{opt expdist(distname)} requests that distance between input and near features be exported as a matrix to the file or matrix {it:distname}.

{phang}
{opt expto(Stata|tab|csv|Mata)} specifies whether the distance matrix should be exported to a Stata matrix or a tab delimited, a csv, and a Mata file. 
Note that if {opt expto(Stata)} is specified, a Stata matrix (loaded in memory) will be created only if the matrix size does not exceed the 
{help matsize} limit of your Stata flavor.

{phang}
{opt sparse(tab|csv|Mata)} specifies that the distance matrix be written as a three-column matrix (row, column, value) to a tab delimited, a csv, 
or a Mata file. {opt expto()} and {opt sparse()} may not be combined, but you must specify one of them when {opt expdist()} is specified. 
...
{phang}
{cmd:des(}{it:des_option,}[{it:des_suboption}]{cmd:)} requests that descriptive statistics for the distance between input and near features
and distance to the nearest neighbors specified with {opt kth(#)} be displayed. {it:des_option} is required when {opt des()} is specified. 
...
Otherwise, lower quartile, median or second quartile, and upper quartile will be displayed as well. With or without the {opt des()} option 
...
{phang}
{opt replace} overwrites existing variables {newvar:1}, {newvar:2}, {newvar:3}, {newvar:4}, {newvar:5}, {newvar:6}, and any variables listed in 
{it:{help newvarlist}} and existing file {it:distname}.
 ...
 r(nearest_mean) =  Average of the distance to the kth nearest neighbors
    ...
    r(mean_dist) =  Average distance between input and near features
     ...
     r(min_dist) =  Minimum distance between input and near features
          r(Obs) =  Number of location pairs between which distance is calculated


{marker examples}{dlgtab:Examples}

{phang}
1) Calculate average test score and proportion of nonwhite for the 3 nearest neighbors 
using Cartesian coordinates

{pmore}{cmd:. nearstat latitude longitude, near(latitude longitude) dist(distname) ///}{p_end}
          {bf:cart contvar(testscore nonwhite) statv(avtest pctnwhite) knn(3) ///}
	  {bf:statname(mean)}
...
{phang}
2) Decipher the id, test score, and race of each observation's nearest neighbor

{pmore}{cmd:. nearstat latitude longitude, near(latitude longitude) dist(distname) ///}{p_end}
          {bf:cart contvar(testscore nonwhite) statv(near_score near_race) ///}
...
{phang}
4) Calculate distance from each rural county to the nearest metropolitan area and record population of the nearest
...
{pmore}{cmd:. nearstat latvar1 longvar1, near(latvar2 longvar2) distvar(distmetro) ///}{p_end}
	 {cmd: contv(popmetro) statv(popnear)}

{phang}
Here popmetro is the variable holding population in each metropolitan area and popnear is a variable name to hold  
...
{pmore}{cmd:. nearstat latvar1 longvar1, near(latvar2 longvar2) distvar(distmetro) ///}{p_end}
          {cmd:contv(popmetro) incdist(incd5) atpop(500000)}  

{phang}
Here incd5 is a variable name to hold the calculated incremental distance

{synoptline}

{phang}
6) Create a variable (nearmetro) holding the population of a county if the county is part of a defined metropolitan
 area or the population of the nearest metropolitan area if the county is a non-metropolitan county.

{p 6 8 2}In addition to the variables in Example 4, you need a variable which holds county population and 
a dummy variable equal to 1 if the county is part of a metropolitan area and zero otherwise.

{p 6 8 2}a) First, set the variable nearmetro equal to the county population:
...
{pmore}{cmd:. nearstat lat long, near(lat long) dist(distvar3) kth(3) r(3958.761) ///}{p_end}
...
{pmore}{cmd:. nearstat lat long, near(lat long) dist(mydist) ncount(nbnei) dband(0 2) ///}{p_end}
	 {cmd: r(3958.761)} 
...
{pmore}{cmd:. nearstat lat long, near(lat long) dist(mydist) des(stat) r(3958.761)} 
...
{phang}
12) Display descriptive statistics for distance between input and near features and for a neighbor count in the distance band: 0<dij<=9
...
{pmore}{cmd:. nearstat latitude longitude, near(latitude longitude) dist(nearestnei) ///}{p_end}
	  {cmd:contvar(pov20_00) statv(neipov00) statn(mean) knn(8)} 
...
{p 6 8 2}d) Finally, calculate distance of each observation to DC:

{pmore}{cmd:. nearstat lat long, near(lat_dc lon_dc) dist(distodc)}  // where lat and long are variables holding the housing coordinates
...
{bf:Jeanty, P.W., M. Partridge, and E. Irwin}. 2009. Estimation of a Spatial Simultaneous Equation Model of Population Migration and Housing Price Dynamics.
{it:Journal of Regional Science and Urban Economics}, Forthcoming.
...
{bf:U.S. census Bureau}. 2002. {it:Census 2000 Gazetteer Files.} {browse "http://www.census.gov/geo/www/gazetteer/places2k.html":http://www.census.gov/geo/www/gazetteer/places2k.html}

{bf:U.S. Census Bureau}. 2008. {it:Cartographic Boundary Files.} {browse "http://www.census.gov/geo/www/cob/bdy_files.html":http://www.census.gov/geo/www/cob/bdy_files.html}
...
{p 4 4 2}The author acknowledges the financial support of the Ohio Agricultural Research and Development Center (OARDC).
...
{bf:Jeanty, P.W.}, 2010. nearstat: Stata module to calculate distance-based variables and export distance matrix to text file. 
