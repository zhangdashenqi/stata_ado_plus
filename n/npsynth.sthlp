...
control units based on the predictors of the outcome. 
The nonparamentric imputation of the counterfactual is computed using weights proportional to the vector-distance between the treated unit's
and the controls' predictors, using a kernel function with pre-fixed bandwidth.
...
{phang} {cmd:kern}{cmd:(}{it:kerneltype}{cmd:)} specifies the type of kernel function to use for builfing synthetic weights.  

{phang} {cmd:t_0}{cmd:(}{it:#}{cmd:)} specifies the time in which treatment starts.
...
. npsynth ddva1 $xvars , panel_var(reporter) time_var(year) t0(2000) trunit(11) bandw(0.4) kern(triangular) gr1 gr2 gr3 ///
  save_gr1(gr1) save_gr2(gr2) save_gr3(gr3) gr_y_name("Domestic Direct Value Added Export (DDVA)") gr_tick(5)
...
{psee}
Online: {helpb synth}, {helpb teffects}, {helpb ivtreatreg}, {helpb ctreatreg}
{p_end}
