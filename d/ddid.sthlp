...
{p2colset 5 18 20 2}{...}
{p2col: {hi:ddid} {hline 1}}Pre- and post-treatment estimation of the Average Treatment Effect (ATE) with binary time-varying treatment{p2colreset}{...}
...
{ifin}
{weight}{cmd:,}
{cmd:model}{cmd:(}{it:{help ddid##modeltype:modeltype}}{cmd:)}
{cmd:pre}{cmd:(}{it:#}{cmd:)}
{cmd:post}{cmd:(}{it:#}{cmd:)}
[{cmd:graph}
...
{pstd}{cmd:ddid} estimates Average Treatment Effects (ATEs) when the treatment is binary and varying over time. Using {cmd:ddid}, the user can estimate 
the {it:pre}- and {it:post}-intervention effects by selecting the {it:pre} and {it:post} intervention periods, also by
plotting the results in a easy-to-read graphical representation. Also, in order to assess the reliability of the causal results achieved by the user's
specified model, {cmd:ddid} allows to test both the "common trend" assumption, and the degree of "balancing" achieved by the user's specified model.
Thus, the model estimated by {cmd:ddid} can be seen as a generalization of the Difference-In-Differences (DID) approach to the case of many {it:post-} and {it:pre-}intervention times.


{phang} According to the {cmd:ddid} syntax:
...
{phang} {cmd:model}{cmd:(}{it:{help ddid##modeltype:modeltype}}{cmd:)} specifies the estimation model, 
...
{pstd}
{cmd:ddid} creates a number of variables:
...
{pmore}
{inp:_D_F1, _D_F2, ..., _D_Fp}:  are the leads of the treatment variable, with {it:p} equal to {it:#} in the {it:post(#)} option



{pstd}
{cmd:ddid} returns the following scalars:
...
{pstd} - Before running {cmdab:ddid}, check that {helpb coefplot} is installed, or install it by typing:
...
{phang}
Jann, B. 2014. {it:A new command for plotting regression coefficientsand other estimates}. 12th German Stata Users Group meeting,
...
{phang}Istat{p_end}
{phang}Italian Institute of Statistics{p_end}
