...
*cap pr drop dsginideco
pr def dsginideco , rclass sortpreserve byable(recall)

  version 8.2
  syntax varlist(min=2 max=2 ts numeric) [if] [in]  [fweight aweight] [ , ///
  ...
  qui su  `inc0' [`weight'`exp'] if `touse' , meanonly
  loc mn0 = r(mean)
  local N = r(N)
  local sum_w = r(sum_w)
  qui su  `inc1' [`weight'`exp'] if `touse' , meanonly
  loc mn1 = r(mean)
  qui su  `dinc' [`weight'`exp'] if `touse' , meanonly
  loc mnd = r(mean)
  loc pi = (`mn1'-`mn0')/`mn0'
  loc  j 0
  foreach v of numlist `parameters' {
    ...
    if (_rc==0) {
      _sgini `inc1' [`weight'`exp'] if `touse', pvar(`p1') param(`v')  `welfare'
      local G1_`j' = r(coeff)
      _sgini `inc0' [`weight'`exp'] if `touse', pvar(`p0') param(`v')  `welfare'
      local G0_`j' = r(coeff)
    }
    else {
      _sgini `inc1' [`weight'`exp'] if `touse', gen(`p1') param(`v')  `welfare'
      local G1_`j' = r(coeff)
      _sgini `inc0' [`weight'`exp'] if `touse', gen(`p0') param(`v')  `welfare'
      local G0_`j' = r(coeff)
    }  
    if ("`kakwani'"!="") {
      _sgini `dinc' [`weight'`exp'] if `touse', pvar(`p0') param(`v') `welfare'
      local Cdinc_`j' = r(coeff)
    }  
    _sgini `inc1' [`weight'`exp'] if `touse', pvar(`p0') param(`v') `welfare'
    local C1_`j' = r(coeff)
