...
*! version 1.0.6 CFBaum 21oct2003 from official
*! version 1.0.5   17nov2002
* 1.0.6: allow use with onepanel

program define dfgls2, rclass sortpreserve
	version 8
...
*/
		...
		if `trend' {
			capture qui reg  `y' `iota' `qdtrd' if `touse', noc
			if _rc > 0 {
				if _rc==2000 {
					di as err "no observations"
				}
			...
			capture qui reg `y' `iota' if `touse', noc
			if _rc > 0 {
				if _rc==2000 {
					di as err "no observations"
				}
...
/* markout one more to account for lag of delta */
	...
	di " "
	di as txt "DF-GLS for " as res "`sname'" _col(30) /*
...
/* run the DF regression, no constant nor trend */
	 	capture qui reg D.`dty' L.`dty' if `touse', noc
		if _rc > 0 {
			if _rc==2000 {
				di as err "no observations"
			}
...
/* run the ADF regression, no constant nor trend */
			capture qui reg D.`dty' L.`dty' DL(1/`i').`dty' /*
				*/ if `touse', noc
			if _rc > 0 {
				if _rc==2000 {
					di as err "no observations"
