*! version 1.4 6Mar2015
*! authors Ricardo Mora & Iliana Reggio

program dqd, eclass byable(recall) 
	version 10.1
quietly {
	...
	noi mata: _dqd(`begin',`end',`diff',`t0',"`alpha'","`std_alp'", "`tests'","`p_tests'", )
	...
	noi _dqd_out, alp(`alpha') stdalp(`std_alp') cluster(`cluster') model(`model')  ///
...
capture program drop _dqd_out
program define _dqd_out
	version 10.0
quietly {
...
mata
        ...
        void _dqd_alphaqs(numeric scalar s, numeric scalar q, numeric vector g , numeric matrix V, numeric scalar a, numeric scalar std, numeric vector delta)
        {
        ...
        void _dqd(numeric scalar inicio, numeric scalar fin, numeric scalar diff, numeric scalar t0,          ///  
		...
		for (s=S; s>=1; s--) {
			...
			Var_g=J(1,t0+s,0)\J(t0+s-1,1,0),V[1::t0+s-1,1::t0+s-1]
	        	_dqd_alphaqs(s,q,gamma,Var_g,a=.,std=.,delta1=.)
			if (s<S) {
		...
		for (q=diff; q>=2; q--) {
			...
			Var_g=J(1,t0,0)\J(t0-1,1,0),V[1::t0-1,1::t0-1]
		        _dqd_alphaqs(1,q-1,gamma,Var_g,a=.,std=.,delta1=.)
