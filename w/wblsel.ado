...
program define wblsel;
	...
	quietly replace `lnf' = `theta2af' + ln(`durdep') + (`durdep'-1)*ln(`lambda2'*$ML_y2) 
				- (`lambda2'*$ML_y2)^`durdep' - `lambda1'
		 		+ ln(1+`rhostar'*(2*exp(-($ML_y2*`lambda2')^`durdep')-1)*(exp(-`lambda1')-1)) 
			if $ML_y1 & !`_rtcnsr';

	quietly replace `lnf' = ln(1 - (1-exp(-`lambda1')) - (1-exp(-($ML_y2*`lambda2')^`durdep')) 
				+ ((1-exp(-`lambda1'))*(1-exp(-($ML_y2*`lambda2')^`durdep'))*(1+`rhostar'*exp(-($ML_y2*`lambda2')^`durdep'-`lambda1'))) )
			if $ML_y1 & `_rtcnsr';
