*! version 1.0.3  10nov2015
*! author: Federico Belotti
*! see end of file for version comments

program define ttab, eclass 
    syntax varlist(min=1) [if] [in], by(varlist) [ SDshow tshow over(varname) over2(varname) TOFILE(string) ttest(string) ///
	...
	forvalues ov1=1/`nlev_over1' {
		forvalues ov2=1/`nlev_over2' {
			...
			foreach var of local by {
				...
				if "`sdshow'"=="" {
					local sqrt_pop_1 "/sqrt(r(N_1))"
					local sqrt_pop_2 "/sqrt(r(N_2))"
				}	
				
				mat `_ttest'[`i',2] = r(sd_1)`sqrt_pop_1'
				mat `_ttest'[`j',2] = r(sd_2)`sqrt_pop_2'
				if "`tshow'"!="" mat `_ttest'[`k',2] = r(p)
				else  mat `_ttest'[`k',2] = r(se)
...
!* version 1.0.1 Apr 2013 
!* version 1.0.2 Jul 2013 over() and over2() added
!* version 1.0.3 Nov 2015 option sdshow added - thanks to Roman Fudickar
