*! version 1.0.5,  06Aug2013 - user must specify retransf method after tpm with regress, log
	...
	if "`scores'" != "" {
		...
		foreach stubvar of local varn {
			if `__i' == 1 & "`eq1'" == "logit" {
				...
				qui logit_p `:word 1 of `vtyp'' `stubvar' if `touse_tpm' == 1 & `touse' == 1, eq(`eq1') scores
				label variable `stubvar' `"equation-level score:tpm (logit)"'
			...
			if `__i' == 1 & "`eq1'" == "probit" {
				...
				qui probit_p `:word 1 of `vtyp'' `stubvar' if `touse_tpm' == 1 & `touse' == 1, eq(`eq1') scores
				label variable `stubvar' `"equation-level score:tpm (probit)"'
				if "`e(offset_probit)'"!="" & "`nooffset'"=="" qui eret local offset "`e(offset_probit)' for probit"
			}
			if `__i' == 2 & "`=trim(regexr("`eq2'", "_log", ""))'" == "regress" {
				tempvar xb_`eq2'
				qui _predict double `xb_`eq2'' if `touse_tpm' == 1 & `touse' == 1 & `e(depvar)'>0, nooff eq(`eq2') xb
				if "`eq2'" == "regress_log" qui gen `:word `__i' of `vtyp'' `stubvar' = ln(`e(depvar)') - `xb_`eq2'' if `touse_tpm' == 1 & `touse' == 1  & `e(depvar)'>0
				else qui gen `:word `__i' of `vtyp'' `stubvar' = `e(depvar)' - `xb_`eq2'' if `touse_tpm' == 1 & `touse' == 1 & `e(depvar)'>0 
				qui replace `stubvar' = 0 if `stubvar' == .
				label variable `stubvar' `"equation-level score:tpm (`eq2')"'
			}
			if `__i' == 2 & "`eq2'" == "glm" {
					qui{
						if "`e(prefix)'"=="svy" {
							...
							*** related to the second part of the model to fit -margins- requirements
							...
							tempvar __esample 
							qui gen `__esample' = (`touse_tpm' == 1 & `touse' == 1 & `e(depvar)'>0)
							...
							predict `:word `__i' of `vtyp'' `stubvar' if `touse_tpm' == 1 & `touse' == 1 & `e(depvar)'>0, `nooffset'  scores
							replace `stubvar' = 0 if `stubvar' == .
							label variable `stubvar' `"equation-level score:tpm (glm)"'
							qui est restore ___tpm
							if "`e(offset)'"!="" {
...
program define CoMpUtE, eclass
	...
	*** It's better if -tpm- returns an error when is used with -glm- families other than gamma or gaussian and link other than: id, log, pow
	...
	*** Combined
	...
	label var `varn' "tpm combined expected values"
