...
*! version 1.2.1 16nov2012  - solved svy check problem adding tpm_svy_check.ado
...
*! 							- all -glm- family and links are allowed now (in the tpm_p.ado too)
...
*! version 1.1  4aug2012 - moremata mm_which inside tpm.ado. svy bugFix and option suest added
...
program define tpm, eclass byable(onecall) sort prop(ml_score swml svyb svyj svyr mi)
	...
	if "`s(exit)'" != "" {
		`_vv' ereturn local cmdline `"tpm `0'"'
		exit
	}
	
    if replay() {
		if "`e(cmd)'" ~= "tpm" error 301
		_tpm_display `0'
	}
...
*** TPM command
program define tpm_est,  eclass byable(recall) sortpreserve
version 11
...
if "`paren'" == "" {
	...
	syntax varlist(min=1 numeric fv) [if] [in] [iw aw pw], ///
					  ...
					  [ vce(passthru) Robust CLuster(string) /// vce options 
		 			  LEVel(cilevel) heteroskedastic(string) SUEST *]
...
if `check_types' != 2 {
	di as error "tpm allows only logit or probit as 1st part and glm or regress as 2nd part." 
	error 198
} 
...
foreach vcepart of local vce_parts {
	local check_vce = regexm("``vcepart''", "vce") + ///
		...
		regexm("``vcepart''", "^cl")       
	if `check_vce' >= 1 {
		di as error "robust, cluster() and vce() options must be specified using tpm syntax."
		error 198
	} 
}
if "`robust'" != "" {
...
di _n "Fitting `second' regression for second part:"
if "`second'" == "regress" {
...
else mata: names2 = regexr(names2, "`lhs'", "`second'")
...
if "$ChEcK_SvY_TpM___"!="" {
	...
	** this is needed for a correct CI display
	...
	if `___rcstrata1' == 0 & "$ChEcK_SvY_TpM___"!="" {
...
*** Display results ***
_tpm_display, level(`level') orf(`_for') ors(`_sor') `diopts'
***********************
...
prog define _tpm_display, eclass
...
///////////////////////////
	...
	#delimit ;
		...
		.`tabdisp'.sep, top;
		_tpm_coef_table_header, part(`:word 1 of `e(eqnames)'');
	#delimit cr
	...
	if regexm("`:word 2 of `e(eqnames)''", "regress") {
		...
		_tpm_coef_table_header, part(`eq2_name')
	}
	else {
		di _n as result "Part 2: `:word 2 of `e(eqnames)''"
		.`tabdisp'.sep, top
...
program _tpm_coef_table_header
	version 9
	if !c(noisily) {
		exit
	}
