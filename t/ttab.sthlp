...
{synopt :{opt tshow}}displays the t-test statistic (and related p-value, if requested through the {help estout} option {opt se}) instead of the means' difference (and its related standard error).{p_end}
{synopt :{opt sdshow}}displays the standard deviations of variables specified in {it:varlist} instead of their standard errors.{p_end}
...
{pstd}
It is worth noting that if the option {opt sdshow} is specified, the {cmd:estout}'s sub-option {opt star} will obviously display the wrong number of stars. 
The same is true when the {cmd:ttab}'s option {opt tshow} is specified. 

{title:Example}

{cmd:webuse byssin}

- Display the {it:pop} average by the {it:smokes}-defined groups and their statistical significance using stars:
{cmd:ttab pop, by(smokes) estout(c(b(fmt(2) star)) mlab(,none) collab(,none))}

- Display the {it:pop} average by the {it:smokes}-defined groups, related standard errors and their statistical significance using stars:
{cmd:ttab pop, by(smokes) estout(c(b(fmt(2) star) se(fmt(3) par)) mlab(,none) collab(,none))}

- Display the {it:pop} average by the {it:smokes}-defined groups and related standard deviations:
{cmd:ttab pop, by(smokes) sdshow estout(c(b(fmt(2)) se(fmt(3) par)) mlab(,none) collab(,none))}
