...
{phang}
{cmd:extreme plot} [{it:{help varname}}] [{cmd:,} {it:plot_options}]
...
Cox-Snell correction is not defined: formally between -1 and -1/3, in practice between -1 and about -0.2. Also, it works for models with constraints, including the {cmd:gumbel} constraint. When bootstrapping
the bias correction, {cmd:extreme} also automatically bootstraps the variance-covariance matrix of the estimate.

{pstd}
When bootstrapping VCE or bias correction, the precise results depend on the initial state of Stata's pseudorandom number generator. See {help set_seed:set seed}. {cmd:extreme} displays this state
...
{pstd}
You can request multiple plots in one {cmd:extreme plot} command line, as in {cmd:extreme plot, qq pp density xiprofile(-.2 .2)}. Most plot types do not affect the stored numerical estimation 
results. However, requesting {opt xiprof:ile()} or {opt retprof:ile()} will cause {cmd:extreme} to save matrices named {cmd:e(xiprofileCI)} and {cmd:e(retprofileCI)} holding the confidence
intervals derived through graphing the profile likelihood.
...
ML-estimated distribution (Horwitz 2001, section 3.1). {cmd:small(bs)} also automatically bootstraps the VCE during this process. The optional {opt r:eps(#)} suboption controls the number of replications; the default is 50. As an example, suppose ML estimates xi=0.27 on a 
small sample. It is 
...
{phang}{opt mrl}[{cmd:(}{it:{help numlist}}{cmd:)}] requests a mean residual life diagnostic plot, which graphs the average amount by which observations above a threshold
exceed it, as a function of the threshold. If the tail of a distribution is well-approximated by the GPD above some threshold, then the MRL plot should be linear above that point 
(Coles 2001, section 4.3.1). Among {cmd:extreme plot} types, {cmd:mrl} is anomolous in being available before estimation and, by the same token, requiring a 
{it:{help varname}} before the command line's main comma. The {it:{help numlist}} indicates at which thresholds the mean residual should be computed. If the {it:{help numlist}}
is omitted, all observed values of the variable
are used, which can be time-consuming in large data sets. If exactly two values are provided, these are taken as the minimum and maximum with 100 
steps in between; missing values for either bound are interpreted as the variable's minimum or maximum. Otherwise, the mean residual is computed only at the listed values.

{phang}{opt pp} plots modeled versus empirical cumulative distribution functions. If the model is a good fit, the plotted points should track closely to a 45-degree
line. See {manlink R diagnostic plots} and Coles (2001, section 3.3.5). The "empirical cumulative distribution function" is essentially the (weighted) share of the observations smaller or equal to
a given observation. E.g, if there are 99 observations, the values of the empirical cdf are 0.01, 0.02...0.99. (This formulation avoids working with the empirical cdf to 0 or 1, which could
correspond to infinite values of the modeled variable.) The modeled cdf for each observation is computed using the estimated parameters. 

{phang}{opt qq} plots modeled versus empirical cumulative quantiles of the distribution of the modeled variable. Again, if the model fits well, the plotted points should follow a 45-degree
line. See {manlink R diagnostic plots} and Coles (2001, section 3.3.5). The empirical quantiles are just the values of the modeled variable, in order. The modeled quantiles result from
applying the inverse cumulative distribution function to "empirical cdf" values listed above, using the estimated parameters. For non-stationary models, the data are 
pre-transformed to the Gumbel distribution, as described in Coles (2001, section 6.2.3).

{phang}{opt ret:urn} requests a graph of the return level as a function of the return period, as predicted by the fitted model, and compares that to the pattern of extremes
in the data. See Coles (2001, section 3.3.5). E.g., if the model fit implies that a category 4 or higher hurricane occurs every 20 years, then one should appear in the data about
...
{phang}{cmdab:xiprof:ile(}{it:{help numlist}}{cmd:)} requests a likelihood profile plot for the shape parameter xi. The model is repeatedly estimated with xi each time constrained to a different value. The
maximized (profile) log likelihood is plotted against xi. This can provide a more reliable basis for constructing confidence intervals. The coverage of the confidence interval
is controlled by the {cmdab:l:evel()} option. The derived confidence interval is stored in the matrix {cmd:e(xiprofileCI)}. The {it:{help numlist}}} is parsed as with the
{cmd:mrl} option, except that missing values are not allowed. Available for only for models that are stationary in xi.

{phang}{cmdab:retprof:ile(}{it:{help numlist}}{cmd:,} {opt per:iod(#)}{cmd:)}, analogous to the previous option, requests a likelihood profile plot for the return level. The model is repeatedly estimated with the return level each time constrained to a different value. The
maximized (profile) log likelihood is plotted against the return level. The derived confidence interval is stored in the matrix {cmd:e(retprofileCI)}. The required suboption {opt per:iod(#)}{cmd:)} specifies the return period. Available for stationary models only.
...
{phang}. {stata "use http://fmwww.bc.edu/repec/bocode/p/portpirie"}{p_end}
{phang}. {stata "extreme gev SeaLevel"}{p_end}
{phang}. {stata "extreme plot, xiprofile(-.3 .3) retprofile(4.15 4.6, period(10))"}{p_end}
...
{phang}. {stata "extreme gev SeaLevel, muvar(Year) small(cs)"} // Cox-Snell small-sample correction for non-stationary GEV model{p_end}
