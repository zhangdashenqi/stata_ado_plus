...
syntax varlist(min=2 max=2 numeric) [, MInimum(real -.12345) MAximum(real .12345) Quadratic Inverse Fieller Level(cilevel)]
tokenize `varlist'
local var1="`1'"
local var2="`2'"
...
if (`t_min')*(`t_max')>0 {
  ...
  di
  di as text "Extremum outside interval - trivial rejection of H0"
  }
