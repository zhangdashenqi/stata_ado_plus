...
/*
	...
	gen double `varn'= (norm(-`COST'*`sigma1'+`z')) /*
			*/ /norm(`z') /*
...
*/
	...
	gen double `zl'  = invnormal(1 - ((1- `level'/100)/2)*[1-normal(-`z')])        
	gen double `varn'_l= exp(-`COST'*`mu1' - `zl'*`sigma1')
	
	gen double `zu'  = invnormal(((1- `level'/100)/2)*[1-normal(-`z')])        
	gen double `varn'_u = exp(-`COST'*`mu1' - `zu'*`sigma1')
