*! version 1.1.2     29nov2004    C F Baum / Fabian Bornhorst
...
* modified by arlion, add return value of  t-value

program define levinlin_a, rclass
	version 7.0
...
* 4B29: guard against no unit variation
	su `vvar', meanonly
	if r(mean)==0 {
...
* LLC eqn 2 (LL eqns 2, 3, 4)
    		reg D.`vvar' `tps`i'' `trend' if `id' == `i' & `touse' `det' 
			capt drop `hold' 
			predict double `hold' if e(sample), r 
		 	replace `eps' = `hold' if e(sample)
		 	reg L.`vvar' `tps`i'' `trend' if `id' == `i' & `touse' `det' 
			capt drop `hold'
			predict double `hold' if e(sample), r 
...
* 2A21: calculate as signed	
	...
	*/ %11.5f tstar _col(40) %9.4f pval
	...
	return scalar lavg=lavg
      return scalar tvalue =tee         /*add by arlion, 2005.7.4*/
