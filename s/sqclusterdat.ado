*! version 1.8 März 10, 2008 @ 15:13:20 UK
...
program _sqclusterdata 
    quietly {
		...
		by `varlist' (`_dta[SQiis]'), sort: gen _SQid = 1 if _n==1
		drop `varlist'
		replace _SQid = sum(_SQid)
		label var _SQid "key for -sqclusterdata, return-"
		sort _SQid
		save _Sqclusterdata, replace
		
		// Keep one Sequence of each type
		by _SQid: gen _SQn = _N
		label var _SQn "Sequence frequency"
		by _SQid: keep if _n==1
		sort _SQid
