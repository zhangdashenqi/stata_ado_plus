...
prog define splagvar
	...
	syntax [varlist(default=none)] [if] [in], [ind(varlist) WName(str) WFrom(str) order(integer 1) ///
								...
								qvar(varlist numeric) qname(namelist) favor(str)]
	marksample touse
	mata: mata clear
	if "`varlist'`ind'`qvar'"=="" {
		di 
	...
	if "`varlist'`ind'"!="" & `:word count `wname' `wfrom''!=2 {
		di as err "Both {bf:wname()} and {bf:wfrom()} required when you specify {bf:varlist1} or {bf:ind(varlist2)}" 
		exit 198
	}
	if "`ind'"=="" & `order'>1 {
		di 
	...
	if "`varlist'"!="" {
		foreach var of local varlist {
		 	conf v `var'
			Confnewvar wy_`var' `replace' // by default created spatially lagged variables have wy_ as prefix
		}
	...
	if "`ind'"!=""  {
		foreach var of local ind {
			conf v `var'
			Confnewvar wx_`var' `replace' 
			if `order'==2 | `order'==3 {
				Confnewvar w2x_`var' `replace'
				if `order'==3 Confnewvar w3x_`var' `replace'  
			}
		}
	}
	...
	if "`qvar'"!="" & "`qname'"!="" {
		...
		if `nv'!=`:word count `qname'' {
 		...
 		forvalue i=1/`nv' {
			...
			tempvar cum pctrank
			cumul `xv`i'', gen(`cum') if `touse' // generate empirical cumulative distribution, no need to calculate rank or sort the data
	...
	if "`varlist'"!="" | "`ind'"!="" {
		...
		cap drop weird_w* 
		mata: lagmyvar("`varlist'", "`ind'", "`touse'")
	}
	if "`plot'"!="" | "`moran'"!="" {
		if "`plot'"!="" local morvar `plot'
		if "`moran'"!="" local morvar `moran'
		mata: CalcMoran("`morvar'", "`touse'")
		if "`moran'"!="" {
			tempname morvec
			mat `morvec'=morstat
		}
	...
	* Renaming the variables
	if "`varlist'"!="" {
		local j=1
		foreach var of local varlist { 
			ren weird_wy`j' wy_`var'
			lab var wy_`var' "Spatially Lagged `var'"
			if `"`plot'"'!="" & "`plot'"=="`var'" {
	...
	if "`ind'"!="" {
		local i=1
		foreach var of local ind { 
			ren weird_wx`i' wx_`var'
			lab var wx_`var' "First order spatially lagged `var'"
			if `order'==2 | `order'==3 {
				ren weird_w2x`i' w2x_`var'
				lab var w2x_`var' "Second order spatially lagged `var'"
				if `order'==3 {
					ren weird_w3x`i' w3x_`var'
					lab var w3x_`var' "Third order spatially lagged `var'"
				}
			}
			local i=`i'+1
		}
