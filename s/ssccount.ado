...
if c(rc) != 0 {
  ...
  forval i = `two' / `tono' {
    ...
    if c(rc) != 0 {
      display ""
      display as error "Warning: datasets from " %tmm_CY `i' " and beyond are not yet available to download;" _newline "datasets up to " %tmm_CY `i'-1 " have been downloaded."
    }
  }
display ""
}
...
if "`graph'" != "" & "`author'" == "" & "`package'" == "" {
  ...
  if `r(r)'==1 & `r(c)'==1 { 
    twoway (line npkghit mo, lc(gs10)) (lowess npkghit mo, lc(gs10) lp(l)) , ytit("Number of hits") ylab(,format(%9.0f)) xlab(,angle(45)) ylab(,angle(0))
  }
  else if `r(r)'==1 & `r(c)'>1 {
    noisily twoway (line npkghit mo, lc(gs10)) (lowess npkghit mo, lc(gs10) lp(l)) , by(author,note("")) ytit("Number of hits") ylab(,format(%9.0f)) xlab(,angle(45)) ylab(,angle(0))
  }
  else if `r(r)'>1 & `r(c)'==1 {
    noisily twoway (line npkghit mo, lc(gs10)) (lowess npkghit mo, lc(gs10) lp(l)) , by(`command',note("")) ytit("Number of hits") ylab(,format(%9.0f)) xlab(,angle(45)) ylab(,angle(0))
  }
  else if `r(r)'>1 & `r(c)'>1 {
    noisily twoway (line npkghit mo, lc(gs10)) (lowess npkghit mo, lc(gs10) lp(l)) , by(author `command',note("")) ytit("Number of hits") ylab(,format(%9.0f)) xlab(,angle(45)) ylab(,angle(0))
  }
}
