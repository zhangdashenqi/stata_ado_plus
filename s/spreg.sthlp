...
{pstd}
{cmd:spreg} requires {cmd:spmat} and {cmd:spivreg} to be installed.
To install {cmd:spmat}, type {cmd:ssc install spmat} from a web-aware Stata.
To install {cmd:spreg}, type {cmd:ssc install spreg} from a web-aware Stata.
...
See {it:{help spreg##technical_note:Technical note}} below for more details.
