...
tempvar Bw D DE DF1 DumE DW DX_ DY_ E e dcs XQX_
tempvar EE Eo Es Ev Ew Hat ht idv itv LE LEo P Q SBB Sig2 SSE
tempvar SST Time tm U U2 Ue wald waldm waldp weit Wi Wio WS X X0 Bo
tempvar Xb XB Xo XQ Yb Yh Yh2 Yhb Yho Yho2 Yt YY YYm YYv Yh_ML Ue_ML Z
tempname A B b B1 b1 b2 Beta BetaSP BOLS Bx mh J K L mfxe IPhi Ls Z0
tempname Cov D den DVE DVNE Dx E E1 EE1 Eg Eo Eom Ew F h Hat idv itv M
tempname P Phi Pm q Q Qr q1 q2 In WS D WW eVec Xo kbm mfxlin mfxlog lf kb
tempname Sig2 Sig2b Sig2o SSE SST1 SST2 Wi1 Wio WY X X0 XB V Dim Sig2o1 rRo
tempname Vec vh VM VP Vs W W1 W2 Wald We Wi xq Xx Y Yh YMB YYm YYv Z olsin
tempname N DF kx kb NC NT Nmiss llf wmat xAx xAx2 TRa2 wWw1 xBx Z1 Zo Ue
di
scalar `N'=_N
scalar `NC'=`nc'
scalar `NT'=_N/`NC'
...
if inlist("`mfx'", "lin", "log") {
tempname XMB YMB XYMB B spmfx spmfxe YMB1
tokenize "`SPXvar'"
qui mean `SPXvar'
matrix `XMB'=e(b)'
qui summ `yvar'  
...
matlist `mfxlin' , title({bf:{err:* Linear:}} Marginal Effect - Elasticity - {bf:Spatial Panel - {bf:(Model= {err:`model'})} *}) twidth(10) border(all) lines(columns) rowtitle(Variable) format(%18.4f)
...
matlist `mfxlog' , title({bf:{err:* Log-Log:}} Elasticity - Marginal Effect - {bf:Spatial Panel - {bf:(Model= {err:`model'})} *}) twidth(10) border(all) lines(columns) rowtitle(Variable) format(%18.4f)
