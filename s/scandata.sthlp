...
{title:Syntax}
{p 8 17 2}
{cmd:scandata} {ifin}  [{cmd:,} {it:options}]
...
{synopt:{opt um:laut}} checks for variable names, labels and value labels containing mutated vowels  {p_end}
{synopt:{opt up:percase}} checks for variable names containing upper-case characters {p_end}

{synopt:{opt all: }} runs all checks above  {p_end}
{synopt:{opt cor:rect}} corrects upper-case letters and/or mutated vowels {p_end}
...
{p 4 8 2} {opt length(length_options)} checks if any variable name, value or variable label in the dataset contains more than a certain amount of characters. The exact amount can be specified by using the {it:length_options}
 {opth vallabel(#)} (default is 119 (120 is the maxium value label length in SPSS)), {opth varlabel(#)} (default is 79) and {opth varname(#)} (default is 8). If no values are entered into the brackets the default values will be used.
  In order to control just for one of those three possibilities the unnecessary options can be simply left out.

{p 4 8 2} {opt nolabel} crawls through every value coded in the dataset checking if a label is assigned. This is useful, since little typos in programming of questionaires can cause unlabled values. Furthermore,
it can be used to identify wildcodes. To avoid unnecessary checks only those variables are scanned which contain a value label. So, unlabeled values can only be found if other values have been assigned to labels already.

{p 4 8 2} {opt noprint} can be used to create a short summary where only counts of variables having been found are reported.
...
{p 8 8 2} The second criterion {opth crit2(#)} looks for values having a distinct high frequency and therefore might hint at some type of program or filter error. Like for the first criterion the threshold being used can be modified,
 but has to be a real number between zero and one. Here, {cmd:scandata} marks all variables which have certain value frenquencies above the chosen threshold (the default is 0.95 and therefore 95 percent).

{p 8 8 2} The third criterion {opth crit3(#)} looks for values having rather small observations hinting at some kind of wildcodes or program/filter error. Therefore, {opth (#)} has to be an integer setting how many and less
 observations are supposed to indicate an error in a variable. The default threshold is one.

{p 4 8 2} {opt umlaut} goes through every variable name, label and value label and checks for existing mutated vowels. Concerning transformation in other formats or downwards compatibility to former Stata versions it can be useful
...
{p 4 8 2} {opt uppercase} accounts for checks regarding upper-case characters in the variable name. Upper-case characters, especially when mixed with lower-case characters, can be an issue when it comes to transformation in other formats.
If {opt correct} is named as well, all upper-case characters are turned into lower-case characters.
...
{p2col 5 20 21 4 :{cmd:r(umlautname)}} list of variables with mutated vowels in variable names  {p_end}
{p2col 5 20 21 4 :{cmd:r(umlautvar)}} list of variables with mutated vowels in variable labels  {p_end}
{p2col 5 20 21 4 :{cmd:r(umlautval)}} list of numeric variables with mutated vowels in value labels  {p_end}
