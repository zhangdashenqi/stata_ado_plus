...
version 11.0
syntax varlist [aw fw iw pw] , nc(str) [WMFile(str) diag ///
...
matrix `In'=I(`N')
di _dup(78) "{bf:{err:=}}"
di as txt "{bf:{err:* Spatial Panel Autoregressive Generalized Least Squares Regression (SPGMM)}}"
di _dup(78) "{bf:{err:=}}"
qui cap drop Time
...
qui spglsxts `yvar' `SPXvar' `wgt' , gmm(`gmm') `noconstant' `vce' `robust' `dn'
scalar `llf'=e(llf)
matrix `Beta'=e(b)
matrix `Cov'= e(V)
...
program define spglsxts , eclass 
 version 11.0
syntax varlist [aw fw iw pw] , [gmm(int 1) NOCONStant vce(passthru) DN ROBust coll tolog]
tempvar `varlist'
gettoken yvar xvar : varlist
 if "`coll'"=="" {
