...
{p 5 5 6} 
{opt pred:ict(new_var)} {opt res:id(new_var)} {opt l:evel(#)} {opt coll zero tolog} {opth vce(vcetype)} {err:]}{p_end}
...
{bf:{err:* (1) Spatial Panel Fixed Effects Lag Model}}
 {stata spregfext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(lin) tests}

{bf:{err:* (2) Spatial Panel Fixed Effects Durbin Model}}
 {stata spregfext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) tests}

 {stata spregfext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) aux(dcs1 dcs2 dcs3) tests}

 {stata spregfext y x1 x2 dcs1 dcs2 dcs3 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) tests}
{hline}

. clear all
. sysuse spregfext.dta, clear
. spregfext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(lin) tests
...
- Schwartz Criterion            SC                =   362.0229
