*! version 1.19.0 September 30, 2010 @ 09:51:24 UK
*! Perform an optimal matching with Needleman-Wunsch-Algorithm
*! Author: Kohler/Luniak,
* Plugin and plugin call: Brendan Halpin
...
version 9.2
	...
	syntax [anything] [if] [in] [, SUBSEQuence(string) IDEALtype(string)       ///
	  ...
	  SUBcost(string) STandard(string) REFseqid(string) full Plugin ///
	  clear replace]
	...
	local plugin = cond("`plugin'"=="plugin",1,0)
   if `plugin' local full 1
	...
	quietly {
		...
		// Levels needed in sqomplug.ado (change by BH)
      ...
      matrix levels = `levels'
		...
		if "`subcost'" == "" local subcost = `indelcost'*2
		capture confirm number `subcost'
		if _rc {
			if "`subcost'" == "rawdistance" | "`subcost'" == "maxprobdistance" | "`subcost'" == "minprobdistance" | "`subcost'" == "meanprobdistance"{
		...
		if `full' {
			...
			// Use Brendan Halpins Plugin (experimental)
			if `plugin' {
				if "`k'"!="0" {
					noi as txt di "Option -k(`k')-, ignored with sqom, plugin"
				}
				noi di as text "Running plugin"
				noi sqomplug `varlist' , subsmat(`subcost') indel(`indelcost') ///
				  pwdist(SQdist)
            }
