capture program drop switch_oprobit
version 10
program define switch_oprobit, eclass sortpreserve 
*!version 1.0.0 cagregory
*!version 1.1.0 update to fix bugs with standard errors of treatment fx
	if replay() {
		if ("`e(cmd)'" != "switch_oprobit") error 301
		Replay `0'
		}
	else Estimate `0'
end
...
syntax anything(equalok) [if] [in] [pweight iweight fweight], treat(varlist) ///
 	[Robust CLuster(varname) level(integer 95)]
...
probit `depvar1' `indvar1' `wgt' if `touse', nocoef
...
ereturn local cmd    "switch_oprobit"
ereturn local depvar "`depvar1' `depvar2'_0 `depvar2'_1 "
ereturn local predict "switch_oprobit_p"
*ereturn display
