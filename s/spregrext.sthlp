...
{p 5 5 6} 
{opt pred:ict(new_var)} {opt res:id(new_var)} {opt l:evel(#)} {opt nocons:tant} {opt coll zero tolog} {opth vce(vcetype)} {err:]}{p_end}
...
{bf:{err:* (1) Spatial Panel Random Effects Lag Model}}
 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sar) mfx(lin) tests}

{bf:{err:* (2) Spatial Panel Random Effects Durbin Model}}
 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) tests}

 {stata spregrext y x1 x2 , nc(7) wmfile(SPWxt) model(sdm) aux(dcs1 dcs2 dcs3) tests}

 {stata spregrext y x1 x2 dcs1 dcs2 dcs3 , nc(7) wmfile(SPWxt) model(sdm) mfx(lin) tests}
{hline}
...
- Schwartz Criterion            SC                =   362.9979
