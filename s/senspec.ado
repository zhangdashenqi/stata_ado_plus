...
qui {;
  ...
  *;
  ...
  bysort `touse' `classgp' `posi': egen `npostype' `neqsame'=sum(`wgt') if `touse';
  ...
  gene `npostype' `ntpos'=sum(`neqsame'*`firpos') if `touse';
  gsort `touse' `classgp' `posi' -`firneg';
  gene `npostype' `nfpos'=sum(`neqsame'*`firneg') if `touse';
  if inlist("`posif'","<",">") {;
    *
     Class variable values equal to threshold test negative
    *;
    tempvar neqpos neqneg neqtemp;
    gene `npostype' `neqtemp'=`neqsame'*`posi' if `touse';
    by `touse' `classgp': egen `npostype' `neqpos'=max(`neqtemp') if `touse';
    drop `neqtemp';
    gene `npostype' `neqtemp'=`neqsame'*(1-`posi') if `touse';
    by `touse' `classgp': egen `npostype' `neqneg'=max(`neqtemp') if `touse';
  ...
  *;
  ...
  gene `npostype' `ntneg'=`sumwgtneg'-`nfpos';
  summ `wgt' if `touse' & `posi';
  scal `sumwgtpos'=r(sum);
  gene `npostype' `nfneg'=`sumwgtpos'-`ntpos';
  *
