...
version 11
syntax varlist(min=2 fv ts) [if] [in] [aweight fweight iweight/] , [ ESTimator(string) Distribution(string) DYNamic POOLed NOCONS COST  ///
                        VCE(string) CLuster(string) Robust Level(cilevel) ///
						GHKDraws(string) CONSTRaints(numlist min=1) PAIRDistance(integer 1) ///
                        EMean(string) Usigma(string) Vsigma(string) ///
						SVFRONTier(namelist) SVEMean(namelist) SVUsigma(namelist) SVVsigma(namelist) SVETA(namelist) ///
...
if "`estimator'"=="pde" & "`dynamic'"!="" & "`rescale'"=="" {
	...
	di ""
	di in gr "If convergence will not be achieved, try using the -rescale- option."
...
if "`estimator'"=="pde" & "`distribution'"=="exponential" & "`u'"=="" & "`forcetechnique'"=="" {
	...
	di in gr "	   If convergence will not be achieved, try to increase the number of iterations or to add the -difficult- option."
}
		...
		if "`distribution'"=="hnormal" {
			...
			*** Consider the constant term to get starting values, then increase nsv_frontier
			...
			if "`svusigma'"=="" {	
				...
				else if "`usigma'"=="" & "`e(cmd)'" =="sfcross" & "`_sfcross_conv'" =="1" mat `init_usigma' = log(e(sigma_u))       
			...
			if "`svvsigma'"=="" {	
				...
				else if "`vsigma'"=="" & "`e(cmd)'" =="sfcross" & "`_sfcross_conv'" =="1" mat `init_vsigma' = log(e(sigma_v))
...
///////////////////////////////////////////////////////////////////
	...
	*** Get Data
	mata: _sftfe_DaTa = _sftfe_GeT_dAtA("`evarlist'", "`touse'", "`lxtset'", "`estimator'", "`distribution'","`nocons'", "`emean'", "`usigma'", "`vsigma'", "`mu_rhs'", "`u_rhs'", "`v_rhs'", "`mu_nocons'", "`u_nocons'", "`v_nocons'", "`fdiff'", "`dynamic'", "`pdetype'", _sftfe_InIt_OpT, &_sftfe_mmsle_util_funct())
	*** Estimation
	noi mata: _sftfe_Results = _sftfe_sf_est_ml("`estimator'", "`distribution'", _sftfe_DaTa, _sftfe_SV, _sftfe_InIt_OpT, _sftfe_PoSt_OpT, "`pdetype'")
...
else eret local upattern "unstructured"
if "`ghkdraws'"!="" {
...
program define ParseDistr
	...
	if ("`__check'" != "hnormal" & "`__check'" != "tnormal") & "`est'"=="fdiff" {
		di as error "Belotti and Ilardi (2012) first difference estimator requires distribution(" in yel "hnormal" in red ") or distribution(" in yel "tnormal" in red ")"
		exit 198
	}
...
*! version 1.2.5  2mar2016 - Release compatible with Stata 11.2, added the undocumented -forcehomo- to allow -pde- also in the homoskedastic case 
                            (due to the way in which gradient is computed in the sftfe_pde_exp.mata, the parametrization uses directly sigma_u and sigma_v, this requires to check for negativeness of sigma_v).
							Now the -mmsle- default is to use simtype(halton) and nsim(n*10) in the homo and nsim(n*30) in the hetero case
							(due to the way in which gradient is computed in the sftfe_mmsle.mata, the parametrization uses directly sigma_u and sigma_v, this requires to check for negativeness of sigma_v).
							From now on, also -mmsle_het- works (more slowly since the gradient should be revised) with time-varying inefficiency factors
