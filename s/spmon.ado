...
* 3.0 Eric Neumayer and Thomas Plümper 20march2010
...
capture findfile mmerge.ado
if _rc {
	di as err _newline "User needs to have {bf}mmerge.ado{sf} installed. Type {bf}net search mmerge{sf} into editor to install."
	exit 601
}
...
if "`reverse_W'"=="reverse_W" {
	quietly {
		...
		use "`yholder'", replace
		mmerge `sourceid' `targetid' `time' using "`Wholder'"
		save "`holder'", replace
		tempvar monadiccont 
		ge `monadiccont'=`spatialvar'*`weightvar' 
		replace `monadiccont'=. if `sourceid'==`targetid'
...
quietly {
	...
	mmerge `sourceid' `time' using `SE_file_monadic_`rowst''
...
di "  with variables {bf}`i'{sf}, {bf}`time'{sf}"
di "  and SE variable {bf}`sename'{sf} saved in current working directory."

if "`merge'"=="nomerge" { 
	di _newline "Open your estimation dataset and use command"
	di _newline "{bf}mmerge `i' `time' using `filename'{sf}"
	di _newline "to merge the spatial effect variable {bf}`sename'{sf} into the estimation dataset."
	restore
}
else {
	restore
	quietly mmerge `i' `time' using `filename'
