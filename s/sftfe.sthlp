...
{phang}
Marginal Maximum Likelihood first-difference estimator (Belotti and Ilardi, 2015; Chen et al., 2014) 

{p 8 16 2}{cmd:sftfe} {depvar} [{indepvars}] {ifin} {weight}
{cmd:, estimator(fdiff)} [{it:{help sftfe##fdiffoptions:fdiff_options}}]

{phang}
Marginal Maximum Simulated Likelihood estimator (Belotti and Ilardi, 2015)

{p 8 16 2}{cmd:sftfe} {depvar} [{indepvars}] {ifin} {weight}
{cmd:, estimator(mmsle)} [{it:{help sftfe##mmsleoptions:mmsle_options}}]

{phang}
Pairwise Difference estimator (Belotti and Ilardi, 2015) 
...
the result outlined in Kotz et al. (2000) (see Chen et al. [2014] and Belotti and Ilardi [2015] for more details). On the other hand, the -pde- uses the Genz (2004) approach to efficiently compute bivariate normal probabilities.{p_end}
{pstd}
...
{pstd}
Belotti, F., Ilardi, G. (2015). Consistent Inference in Fixed-effects Stochastic Frontier Models, Temi di discussione (Economic working papers), Bank of Italy (forthcoming).{p_end}
