...
{p 2}{help spmlreg##syntax:Syntax}{p_end}
{p 2}{help spmlreg##description:General description}{p_end}
{p 2}{help spmlreg##options:Description of the options}{p_end}
{p 2}{help spmlreg##results:Saved Results}{p_end}
{p 2}{help spmlreg##examples:Examples}{p_end}
{p 2}{help spmlreg##acknow:Acknowledgments}{p_end}
{p 2}{help spmlreg##author:Author information}{p_end}
{p 2}{help spmlreg##cit:Citation}{p_end}
...
{phang}
{cmd: spmlreg} {help varlist}{cmd:,} {opt w:eights(weights_name)} {opt wf:rom(Stata|Mata)} {opt e:ignvar(varname)} {opt m:odel(lag|error|durbin|sac)} 
...
{synopt :{opt l:evel(#)}}specify the confidence level{p_end}
...
{synopt :{opt sr2}}display goodness of fit measures for the spatial lag, the spatial durbin, and the general spatial models{p_end}
...
{pstd}{cmd:spmlreg} estimates the spatial lag, the spatial error, the spatial durbin, and the general spatial (or spatial mixed) models by 
maximum likelihood. Optionally, at the end of the estimation process, predicted values of the dependent variable from the spatial lag, the 
spatial durbin, and the general spatial models are generated based on the expression for the conditional expectation of the reduced form since 
...
{pmore}{bf:Note 1:} Options {opt wrho()} and {opt eigwhro()} are necessary when you want to estimate the general spatial model with different spatial 
...
{pmore}By default, goodness of fit measures are calculated when you specify {opt model(error)}. Note also that predicted values for the dependent variable in the spatial 
error model can be obtained using the {help predict} command after estimation:{p_end} 
{pmore2}{bf:. predict y_hat}
...
{col 4}{cmd:e(pred_y)}{col 22}predicted values of dependent variable from the lag, durbin, and sac models
{col 4}{cmd:e(title)}{col 22}title in estimation output
{col 4}{cmd:e(chi2type)}{col 22}Wald or LR; type of model chi-squared test
...
{pmore}{stata "spwmatrix gal using http://fmwww.bc.edu/repec/bocode/c/columbus.gal, wn(W) eignvar(eigvarW) rowstand mataf" :. spwmatrix gal using http://fmwww.bc.edu/repec/bocode/c/columbus.gal, wn(W) eignvar(eigvarW) row mataf}{p_end}
...
{phang}
{bf:Note 3}: For the Columbus crime dataset, {cmd:spwmatrix} can generate the weights matrix and the eigenvalues vector needed in order to use 
{cmd:spatreg}. You need to specify option {opt eignval()} and remove option {opt mataf}.
...
{p 4 13 2}Online: {helpb spatreg}, {helpb sarml}, {helpb spreg} (if installed) 
