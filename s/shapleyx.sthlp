{smcl}
{* 20may2014}{...}
{cmd:help shapleyx}{right:Patrick Royston (after Stas Kolenikov)}
{hline}
...
{pstd}
{cmd:shapleyx} performs (exact additive) decomposition of a sample statistic
...
{pstd}
In other words, {cmd:shapleyx} effectively creates 2^(# factors) patterns of
...
{pstd}
{cmd:shapleyx} is written in the most general way and saves as many results
...
{phang}. shapleyx fp_weight(weight_1 weight_2) foreign i.rep78 mpg length, result(e(r2)):regress price @{p_end}
...
{phang}
1. Since {cmd:shapleyx} can be computationally intensive, you might
...
2. {it:call_to_program} {cmd:@} [{cmd:,} {it:call_options} might in fact
   ...
   non-missing returned values to {cmd:shapleyx}. A returned missing value
   is likely to crash {cmd:shapleyx} at some point.
{p_end}
...
{pstd}
{cmd:shapleyx} saves the following:
...
{phang}
{stata "shapleyx weight i.foreign i.rep78 mpg length, result(e(r2)):regress price @"}
{p_end}
...
Patrick Royston, MRC Clinical Trials Unit at UCL, London; j.royston@ucl.ac.uk
