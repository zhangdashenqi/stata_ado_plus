...
program define sudcd_wbl_aft;

	version 7;
	args lnf theta1 theta2 theta3 theta4;

	tempvar lambda1 lambda2 rho p _rtcnsr P_cond P_marg P_cum P_cens0 P_cens1 e_l1 e_l2;

	quietly gen double `rho' 	= (exp(2*`theta3')-1)/(exp(2*`theta3')+1);
	quietly gen double `p' 		= exp(`theta4');
	...
	quietly gen double `e_l2'	= exp(-(`lambda2'*$ML_y2)^`p');

	quietly gen double `P_cond'	= `e_l1'*[1 + `rho'*(2*`e_l2' - 1)*(`e_l1' - 1)];
	quietly gen double `P_marg'	= `lambda2'*`p'*[(`lambda2'*$ML_y2)^(`p'-1)]*`e_l2';
