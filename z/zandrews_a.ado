...
* 1.0.1  28jan2004
program define zandrews_a, rclass
	version 8
...
*			qui replace `breaktrend'=_n+1-`time' if _n+1- `time'>0 
	...
	di in g _n "Zivot-Andrews unit root test for {result: `var'}"
	di in g _n "Allowing for break in {result:`obr'}"
	di in g _n "Lag selection via {result:`lagtype'}: lags of D.`var' included = `bestlag'"
	scalar dminobs= `timevar'[`minent']
	di in g _n "Minimum t-statistic " %6.3f `minbreak' " at " `rts' dminobs " (obs `minent')"
	di in g _n "Critical values: 1%: " %5.2f `crit01' " 5%: " %5.2f `crit05' 
	return local breaktype= "`brtype'"
	return scalar tmin=`minbreak'
	return scalar crit01=`crit01'
	return scalar crit05=`crit05'
      return scalar breakyear = dminobs
