*! version 1.0.7     20050316     C F Baum / Fabian Bornhorst
...
*  verion is 8.2 originally, now 8.0 modified by Arlion,2005.3.27
*  modified by arlion, 2005.7.10

program define ipshin_a, rclass
	version 8.0
...
* define unit identifier
	...
	scalar tbar = 0
	scalar nt = 0
      tempvar tvec                         /*add by arlion*/
      mat `tvec' = J(`N',1,0)          /*add by arlion*/
      local jj=1                              /*add by arlion*/
	foreach i of local vals {
    	qui { 
* IPS eqn 4.2
    		reg D.`vvar' L.`vvar' `tps`i'' `trend' if `id' == `i' & `touse' 
    		mat b = e(b)
    		mat v = e(V)
    		scalar nt = nt + e(N)
    		scalar tst = b[1,1]/sqrt(v[1,1])
*    		di in r "`i'  tstat " tst
    		scalar tbar = tbar + tst
            mat `tvec'[`jj',1] = tst                  /*add by arlion*/
            local jj = `jj' + 1                            /*add by arlion*/
    		}
	}
...
* 3408 calc as one-tailed test 	scalar pval = 1-norm(abs(psi))
	...
	*/ %9.3f r(cv1) _col(39) %9.3f psi _col(49) %9.3f pval 
	...
	return local determ `text'
      return mat tvec = `tvec'                   /*add by arlion*/
	end		
