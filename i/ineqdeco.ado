...
*	global S_w2 = `w2'[1]
	...
	*************************
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
	  */ & `touse', c(`vk' `meanyk' `lambdak' `thetak' `lgmeank') f(%9.5f)

	noi di "              "
	noi di "Subgroup indices: GE_k(a) and Gini_k "
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
	  */ & `touse', c(`im1k' `i0k' `i1k' `i2k' `ginik')  f(%9.5f)
	...
	* Atkinson between-group inequality (Blackorby et al., eqn (17))
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
	  */ & `touse', c(`edehlfk' `ede1k' `ede2k')  f(%9.5f)
	...
	noi tabdisp `bygroup' if `bygroup'[_n] != `bygroup'[_n+1] /*
	  */ & `touse', c(`whalfk' `w1k' `w2k' `wginik')  f(%9.5f)
