...
using the {helpb listtex} package.
...
The {helpb sencode} and {helpb listtex} packages are downloadable from {help ssc:SSC}.
...
before using {cmd:ingap} and {helpb listtex}.
...
using the {helpb listtex} package (downloadable from {help ssc:SSC}).
...
{p 8 16}{cmd:. listtex make mpg weight, type}{p_end}
...
{p 8 16}{cmd:. listtex make mpg weight price, type}{p_end}
{p 8 16}{cmd:. restore}{p_end}
...
{p_end}
{p 10 21}
help for {helpb listtex}, {helpb sencode}, {helpb sdecode}, {helpb hplot}, {helpb eclplot} if installed
{p_end}
