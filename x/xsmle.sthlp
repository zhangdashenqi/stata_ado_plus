...
{syntab:Other}
{synopt :{opt dlag}}includes lagged dependent variable in the model.{p_end}
{synopt :{opt nocons:tant}}suppress the constant term in the model. Only for {cmd:re} estimator{p_end}
{synopt :{opt noeff:ects}}suppress the computation of direct, indirect and total effects{p_end}
{synopt :{opt nsim(#)}}set the number of simulations to compute the standard errors for the spatial direct and indirect effects{p_end}
...
{synoptline}
{synopt :{opt ind}} for individual fixed effects effects{p_end}
...
{cmd:xsmle} fits fixed or random effects spatial models for balanced panel data. See the {help mi} prefix command in order to use {cmd:xsmle} in the unbalanced case. Consider the following general specification for the spatial panel data model:
		...
		y_it = tau*y_it-1 + rho*W*y_it + beta*X_it + D*Z_it*theta + a_i + gamma_t + v_it 
...
{pstd}
{cmd:i)} the SAR model with lagged dependent variable (theta=lambda=0) 

		y_it = tau*y_it-1 + rho*W*y_it + beta*X_it + a_i + gamma_t + u_it 

{pstd}
where the standard SAR model is obtained by setting tau=0.

{pstd}
{cmd:ii)} the SDM model with lagged dependent variable (lambda=0)

		y_it = tau*y_it-1 + rho*W*y_it + beta*X_it + D*Z_it*theta + a_i + gamma_t + u_it 

{pstd}
where the standard SDM model is obtained by setting tau=0. {cmd:xsmle} allows to use a different weighting matrix for the spatially lagged dependent variable (W) and the spatially lagged regressors (D) together with a different sets of explanatory (X_it) and spatially lagged regressors (Z_it). 
The default is to use W=D and X_it=Z_it.

{pstd}
{cmd:iii)} the SAC model (theta=tau=0) 
...
{pstd}
{cmd:iv)} the SEM model (rho=theta=tau=0) 

		y_it = beta*X_it + a_i + gamma_t + v_it 
		v_it = lambda*E*v_it + u_it

{pstd}			
{cmd:v)} the GSPRE model (rho=theta=tau=0) 
...
{phang}
{opt dlag} includes (time) lagged dependent variable in the model.
...
{phang}
{opt noeffects} suppresses the computation of direct, indirect and total effects.

{phang}
{opt nsim(#)} sets the number of simulations for the Lesage and Pace (2009) procedure to compute the standard errors of the direct and indirect effects
...
{title: Options for the Generalised Spatial Panel Random Effects (GSPRE) model}
...
{synopt :{cmdab:r:obust}}clustered sandwich estimator where {it:clustvar} is {it:panelvar}{p_end}
...
{pstd}When variance-covariance matrix is not positive definite, direct, indirect and total effects standard errors are computed using a modified positive definite matrix as in Rebonato and Jackel (2000).{p_end}
