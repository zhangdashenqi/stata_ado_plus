...
program define xfrac_chk, sclass
	...
	tokenize areg clogit cnreg _cnreg cox ereg fit glm intreg logistic logit poisson probit /*
		*/ qreg regress rreg weibull xtgee staft streg stcox stpm stpm2 stcrreg /*
	...
	*/
	...
	else if "`cmd'"=="stcox" | "`cmd'"=="streg" | "`cmd'"=="staft" | "`cmd'"=="stpm" | "`cmd'"=="stpmrs" | "`cmd'"=="stpm2" | "`cmd'"=="stcrreg" {
						sret local dist 7
	}
	else if "`cmd'"=="intreg" {
						sret local dist 8
	...
	sret local isnorm  = ("`cmd'"=="regress" | "`cmd'"=="fit" | "`cmd'"=="rreg" | "`cmd'"=="areg") 
end
