...
the outcome variable into within-cluster and between-cluster effects (Schunck 2013, Schunck & Perales 2016). This is accomplished by: (i)
...
model and displays a warning. The between-cluster effects are given by the cluster-mean variables beginning with the prefix {cmd:B_} while
the within-cluster effects are given by the cluster-mean differenced variables beginning with the prefix {cmd:W_}. The prefix {cmd:R_} is used to
...
and within-cluster effects of cluster-varying variables on Y{it:ij}. The within-cluster effects are denoted by variables with the prefix {cmd:B_}, the differences between
the between-and within-cluster effects are denoted by variables with the prefix {cmd:D_}, and the random effects are denoted by variables with the prefix {cmd:R_}. 
...
these take the form: {it:_b[B_varname] = _b[W_varname]}. When the {cmd:cre} option is also specified, the tests take the form {it:_b[D_varname] = 0}.
...
{title:References}
 ...
 Schunck, R. & Perales, F. (2016). "Within- and Between-Cluster Effects in Generalized Linear Mixed Models: A Discussion of Approaches and the {it:xthybrid} Command".
 Forthcoming in {it:Stata Journal}
