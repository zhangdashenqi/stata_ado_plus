*! version 1.0   December 13, 2004 Scott Merryman    
*! Based on Luca Nunziata's -xtfptest- and -xtdftest- 
* it is version8.2 originally, modified to 8.0 by arlion
* the original file is named xtfisher, this one is modifiec by arlion with "levels" deleted

program xtfisher_a, rclass
	version 8.0
...
//	qui levels `id' , local(levels)
	...
	foreach l of local levels {
		capture dfuller `varlist' if `id' == `l'  , `trend' `drift' lags(`lags')
		if r(p) != . {	
			local N_g = `N_g' + 1
			}
		if  "`display'" != ""  & r(p) ! = . {
			...
			dfuller `varlist' if `id' == `l'  , `trend'   `drift' lags(`lags')
		}
	...
	*return scalar pval_z =  norm(`Z')
	...
	foreach l of local levels {
		capture pperron `varlist' if `id' == `l' , `trend'  lags(`lags')
		if r(pval) != . {
			local N_g = `N_g' + 1
			}
		if  "`display'" != ""  & r(pval) != . {
			disp ""
			display in gr "Cross-section unit: " `l'
			pperron  `varlist' if `id' == `l'  , `trend'  lags(`lags')
		}
