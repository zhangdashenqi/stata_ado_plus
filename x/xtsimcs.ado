...
*! simulate estimators of dynamic panel data
*! based on
...
syntax newvarlist (min=3 max=5 numeric), Nid(integer) Time(integer) Gamma(real) /*
...
local cor_tt2=(`gr'^2/(1+`gxr'))-`gxr'
	...
	qui {
		...
		/* gen time var */
 		...
 		gen `epsx'=`sx'*invnorm(uniform())     // sigma of x
 		gen `epsy'=`sigma'*invnorm(uniform())  // sigma of y
