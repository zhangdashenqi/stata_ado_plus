...
if ~missing(`"`using'`save'"') local options `"`options' save("`using'`save'")"';
...
quietly {;
	...
	if ~missing(`"`rblanks'"') {;
		...
		while (~missing(`"`rblanks'"')) {;
			...
			local num = trim("`num'");
			if "`num'"=="LAST_ROW" local num = _N - nreq[_N] - 3;
...
forvalues i = 1/`=_N' {;
	...
	foreach var of local vlist {;
   		local datum = `var'[`i'];
		if (`"`datum'"'=="_SKIP_ROW") continue; // rblanks
...
program define Mkemat, rclass;
   	...
   	if ~missing("`sd2'") {;
		...
		forvalues i = 1/`= rowsof(`bbc')' {;
			...
			forvalues j = 1/`= colsof(`bbc') / 2' {;
				...
				local cnames : colnames `tmpC';
				local ceq    : coleq `tmpC', quoted;
				local colnames `"`colnames' "`: word 1 of `: roweq `tmpC', quoted''""';
				matrix `tmpB' = nullmat(`tmpB'), `tmpC';
			}; // j
			matrix rownames `tmpB' = `cnames'_easytofind0 `cnames'_easytofind1;
			matrix roweq	`tmpB' = "`: word 1 of `ceq''";
			matrix  `tmpA' = nullmat(`tmpA') \ `tmpB';
		}; // i
 	 	matrix coleq    `tmpA' =`colnames';
	...
	local cnames_old : colnames `bbc';
	local ceqs_old   : coleq `bbc', quoted;
	local cnames;
	forvalues j = 1/`rc' {;
		local jC = int((`j'-0.1)/(1+`sdc'))*(1+`sdc')+`j';
		local cnames `"`cnames' `: word `jC' of `cnames_old''"';
		local ceqs   `"`ceqs' "`: word `jC' of `ceqs_old''""';
 		forvalues i = 1/`rr'{;
	...
	matrix roweq    `D' = `: roweq `bbc', quoted';
...
program define formateqnames;
	...
	local eq  : coleq `B', quoted;
  	local eq : subinstr local eq "." "`=char(183)'", all;    // mlogit problem
	matrix coleq `B' =`eq';
	...
	if missing(trim(`"`eq'"')) {;
		...
		local s   : list eq - ueq;
		local s   : list ueq - s; // constant equations

		if missing(`"`s'"') exit;

		local cnames : colnames `B';
		forvalues i = 1 / `=colsof(`B')' {;
			if strpos(`"`s'"', `"`: word `i' of `eq''"') & `i' > 1 {;
				local newceqs 	`"`newceqs' "easytofind2""';
				local newcnames `"`newcnames' "`: word `i' of `eq''""';
			};
			else {;
				local newceqs 	`"`newceqs' "`: word `i' of `eq''""';
				local newcnames `"`newcnames' "`: word `i' of `cnames''""';
			};
		};
		matrix colnames `B' = `newcnames';
		matrix colnames `V' = `newcnames';
		matrix coleq 	`B' = `newceqs';
		matrix coleq 	`V' = `newceqs';
...
program define savetodataset;
	...
	forvalues i = 1/`rr' {;
   	...
   	forvalues j = 1/`rc' {;
		...
		local coleq : subinstr local coleq "`=char(183)'" ".", all;
		...
		local cn : subinstr local cn "`=char(183)'" ".", all;
