...
{synopt :{opt a}}the fixed or random-effect{p_end}
{synoptline}
{p2colreset}{...}
...
{phang}
{opt a} estimates {it:a_i}, the fixed or random-effect. In the case of fixed-effects models, this statistic is allowed only when {opt type(ind)}.
