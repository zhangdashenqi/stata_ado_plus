...
*! modified by Arlion, 2007.09.18,搜索"Arlion add"即是我新加的内容
program define xtabond4, sortpreserve byable(onecall)
	quietly query born
	if $S_1 < d(23may2002) {
...
/* end new method */
	...
	while `i' <= `artests' {
		est scalar arm`i' = `arm`i''
            est scalar p_arm`i' = 2*(normprob(-1*abs( (e(arm`i')) ) ))  /*Arlion add*/
		if "`robust'" == "" & `i'==1 {
...
/* do wald test on constant */
	...
	est local cmd "xtabond"
      est scalar sar_p  = chiprob(e(zcols)-e(df_m),e(sargan)) /*Arlion add*/
