...
{p 4 13}{cmd:xtcd2} [{varname}(max=1)] [{cmd:,}{cmdab:noest:imation} {cmd:rho} {cmdab:hist:ogram} {cmd:name({it:string})} ]{p_end}
...
Furthermore by specifying the {cmd:histogram} option, a histogram with the distribution of the cross correlations is drawn.
...
{p 4 4}{cmdab:hist:ogram} plots a histogram of the cross correlations.
...
{p 4 4}{cmd:name({it:string})} saves the histogram.{p_end}
...
{col 8}{cmd: r(cdp)}{col 27} p-value of CD test statistic
...
{p 4}Thanks to Achim Ahrens for providing many helpful comments to the code.{p_end}
