...
* Push -model- option in help file
* Correct typos introduced in 1.01
...
* di "*****  parse the command ***************
	error 0
	syntax varlist(min=1 max=1 numeric) [if] [in]  , /*
*/ [AVerage(string) bar(string) /*
...
*/ L1title(string) Symbol(string) Connect(string) Weight(string) nograph * ]


* di "*****  ***** ***** ***** ***** ***** 

* di "*****  Collect the I and T vars ***** 
	xt_iis `i'
	local ivar "`s(ivar)'"
...
* di "*****  Check the syntax ***** 
	...
	if "`average'" == "" |  "`average'" == "am" {local average = "mean" }
	...
	if _rc { di in gr "Note: There is no point in asking for half-bars if no bars are requested." 
		local half
		}
...
* di "*****  Symbol ***** 
	...
	while length("`symbol'") < `ngrp' & `ngrp' ~= . { local symbol "`symbol'OSTodp" }
...
* di "*****  Connect ***** 
	...
	while length("`connect'") < `ngrp' { local connect "`connect'l" }
...
* di "*****  Collect the Y and T labels ***** 

	local y_vrlab : var label `varlist'
	if "`y_vrlab'" == "" { local y_vrlab = "`varlist'" }
	local y_vllab : val label `varlist'

	local t_vrlab : var label `tvar'
	if "`t_vrlab'" == "" { local t_vrlab = "`tvar'" }
	local t_vllab : val label `tvar'
	

***** ***** ***** ***** ***** ***** 

* di "*****  Carry out the transformations *****
...
* di "*****  Calculate the averages and bars ***********

	tempvar av 
	if "`bar'" ~= "no" { tempvar lb ub }
...
* di "*****  Carry out the back transformations *****
	if "`power'" ~= "1" & "`power'" ~= "0" { 
...
* di "*****  Offset the tvar *****
	...
	else {	qui gen `t2' = `tvar' }
	label var `t2' "`t_vrlab'"
	label val `t2' `t_vllab'
	
***** ***** ***** ***** ***** 

* di "*****  Drop the extra variables & values *****
...
* di "*****  Rename the variables ***** 
	cap rename `lb' lb
		if _rc == 0 { local lb lb  }
	cap rename `ub' ub
		if _rc == 0 { local ub ub  }
	cap rename `t2' t2
		if _rc == 0 { local t2 t2 }
	cap rename `av' `average'
		if _rc == 0 { local av `average'}
	cap noi rename `n' n
		if _rc == 0 { local n n}
cap mac li _t2
cap desc `t2'
...
* di "*****  Label the groups *****
	...
	if _rc {
		...
		if _rc == 0 { local grp_num "`group'" }
		else { encode `group' , gen(`grp_num') }
...
* di "*****  Rename the group average for group `i' *****
...
* di "*****  List the data *****
	...
	if "`list'" ~= "" {
		if  "`average'" == "mean" {
			if "`log'" == "0" { local av_type "Geometric mean" }
			else if `power' == -1 { local av_type "Harmonic mean" }
			else if "`power'" ~= "1" { local av_type = "Transformed mean [x^`power']" }
			else if "`boxcox'" ~= "1" { local av_type = "Box-Cox transformed mean [x^`power']" }
			else if "`log'" ~= "" { local av_type = "Transformend mean [log(x -`log')]" } 
			else { local av_type "Arithmetic mean" }
		}
		else local av_type = "`average'"
	
		di _n "`av_type' of `varlist' " _c

		if "`bar'" == "ci" |  "`bar'" == "rr" { local l = "`level'% " }
		if "`bar'" ~= "no" {
			local up_bar = upper("`bar'")
			di "with bars based on `l'`up_bar'"
		}
		if "`group'" ~= "" & "`by'" == "" { di "by `group' and `tvar'." }
		else if "`group'" ~= "" & "`by'" ~= "" { di "by `group', `by' and `tvar'." }
		else if "`group'" == "" & "`by'" ~= "" { di "by `by' and `tvar'." }
		else di "by `tvar'."
		if "`group'" ~= "" | "`by'" ~= "" { 
			sort `group' `by' 
			by `group' `by' : list `tvar' `n' `av' `lb' `ub' , noobs 
		}
		else  { list `tvar' `n' `av' `lb' `ub' , noobs }
	}
***** ***** ***** ***** ***** 

* di "*****  Weight the points ***** 
...
* set trace on
	...
	if "`show'" ~= "" {
		cap noi di in wh "graph `avlist' `lb' `ub' `t2' `weight', connect(`connect') symbol(`symbol') `xlab' `ylab' `by' `options' `saving' "
	}

	if "`graph'" == "" {graph `avlist' `lb' `ub' `t2' `weight', connect(`connect') symbol(`symbol') `xlab' `ylab' `l1title' `by' `saving' `options'}
*************************************
...
program define centile, rclass
	...
	syntax [varlist] [if] [in] [, CCi /*
		...
		*/ Level(real $S_level) Meansd Normal ]
