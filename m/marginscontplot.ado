*! version 1.1.1 PR 03sep2013
program define marginscontplot
version 12.1
...
syntax [anything(name=xlist)] [if] [in] [, at(string) at1(string) at2(string) ///
 ci Formatlegend(string) MARgopts(string) nograph PLOTopts(string) ///
...
if "`graph'" != "nograph" {
	if "`ci'" == "" {
		line _margin* `x', sort `leg' `plotopts'
	}
	else {
		if `nat2' > 0 {
			local gi
			forvalues i = 1 / `nat2' {
				// Note: `plotopts' ignored in these component graphs,
				// `plotopts' applies to the following graph combine
				tempname g`i'
				twoway (rarea _ci_lb`i' _ci_ub`i' `x', sort pstyle(ci)) ///
				 (line _margin`i' `x', sort lstyle(refline)), legend(off) ///
				 title("`x2' = `lab2`i''") name(`g`i'', replace) nodraw
				local gi `gi' `g`i''
			}
			graph combine `gi', `plotopts'
		}
		else {
			twoway (rarea _ci_lb _ci_ub `x', sort pstyle(ci)) ///
			 (line _margin `x', sort lstyle(refline)), `leg' `plotopts'
		}
	}
}
end
