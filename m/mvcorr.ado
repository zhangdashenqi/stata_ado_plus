...
* 1.0.6 24sep2004 parallel change to mvsumm to correct results with if/in
	...
	, Generate(name)  [ Window(int -1) End ]
	...
	* and hit with -forval-
	qui if "`by'" != "" { 
*		levels `by' if `touse', local(bylist) `missing'
		tempvar group
		...
		forval i = 1 / 	`max' {
			...
			* logic from movsumm
			...
			forval jay = `jaybase'/`jlast' {
				corr `varlist' in `eye'/`kay' 
* correction		if `group' == `i'
*					if `touse' & `by' == `b' 
			        local na = r(N)
	...
	* single timeseries, no loop over panel var
		...
		* logic from movsumm
		...
		local kay = `window'
		forval jay = `jaybase'/`jlast' {
			corr `varlist' in `eye'/`kay' /* if `touse' */ 
			local na = r(N)
			replace `gen' = r(rho) in `jay' if `na' == `window'  
			local ++eye 
		        local ++kay 
		}     	
