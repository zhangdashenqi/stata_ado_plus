...
forvalues `k'=0(1)``npvar'' {
	...
	*/ _column(``col4st'') %`colwd'.4f 2*(1-norm(abs(`matB'[1,1]/sqrt(`matV'[1,1]))))

	matrix `matFT'[``j'',1]=`matB'[1,1]
	matrix `matFT'[``j'',2]=sqrt(`matV'[1,1])
	matrix `matFT'[``j'',3]=`matB'[1,1]/sqrt(`matV'[1,1])
	matrix `matFT'[``j'',4]=2*(1-norm(abs(`matB'[1,1]/sqrt(`matV'[1,1]))))
...
matrix `matRT'[1,4]=2*(1-norm(abs(`matB'[1,1]/sqrt(`matV'[1,1]))))
...
forvalues `k'=0(1)``npvar'' {
	...
	*/ _column(``col4st'') %`colwd'.4f 2*(1-norm(abs(`matB'[1,1]/sqrt(`matV'[1,1]))))
	...
	matrix `matRT'[``j'',4]=2*(1-norm(abs(`matB'[1,1]/sqrt(`matV'[1,1]))))
