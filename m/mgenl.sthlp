{smcl}
{* 2014-09-21 scott long}{...}
{title:Title}

{p2colset 5 14 14 2}{...}
{p2col:{cmd:mgenl} {hline 2}}BETA: Generate variables for plotting with predictions
from {cmd:margins} using local means{p_end}
{p2colreset}{...}
...
{p 4 8 2}
{cmd:mgenl} [{it:if}] [{it:in}]{cmd:,}
 [mgenl-options] [margins-options]

{marker overview}
{title:Overview}

{pstd}
{cmd:mgenl} uses results from {cmd:margins} to create variables for plots.
{cmd:mgenl} has the advantage of allowing marginal effects from multiple models to be combined in the same plot, and for marginal effects from multiple categories to be combined in the same plot.

{title:Options}

    {help mgenl##local:BETA Options for local means}
    {help mgenl##margins:Options for margins}
    {help mgenl##name:Naming variables}
    {help mgenl##stats:Statistics and values generated}
    {help mgenl##meanpred:Variables generate with the meanpred option}
    {help mgenl##model:How mgenl works with various types of models}
    {help mgenl##other:Miscellaneous options}
    {help mgenl##return:Returns}
    {help mgenl##examples:Examples}

{marker mgenl}
{dlgtab:BETA Options for local means}
{p2colset 7 24 25 0}
{synopt:{opt localv:ars(variable-list)}}
...
{synopt:{opt bin:width(number)}}
With of region around value of variable that is varying that is used to
select observations for computing local means. For example, with
{cmd:at(age=(30(10)80)) binwidth(20)} the means of other variables when
{cmd:age=30} are computed for {cmd:age>=20 & age <=40}.
...
{it:<quantity>} describes the {help mgenl##stats:content} contained in
the variable
...
{p 6 6 2}
Variables generated by {cmd:mgenl} include estimates and related statistics from
...
{p2colset 9 24 25 0}
{p 6 6 2}
{cmd:mgenl} has different default behavior depending on
...
{phang2}{cmd:. mgenl, at(lwg=(-2(.5)3)) stub(prlfp)}{p_end}
...
{phang2}{cmd:. mgenl, at(age = (20(5)85) year=3 white=1) stub(prclass)}{p_end}
{phang2}{cmd:. twoway connected prclassCpr1 prclassCpr2 prclassCpr3 }{p_end}
{phang2}{cmd:> prclassCpr4 prclassage}{p_end}

{pstd}
{ul:{bf:Example 3: Plotting variables from multiple mgenl commands}}{p_end}

{phang2}{cmd:. spex logit}{p_end}
{phang2}{cmd:. mgenl, at(inc=(0(5)95) wc=0) stub(wc0)}{p_end}
{phang2}{cmd:. mgenl, at(inc=(0(5)95) wc=1) stub(wc1)}{p_end}
