*! version 1.1.4    23jun2000       C F Baum/V Wiggins (STB-57: sts16)
...
*  1.1.4 corrected z pvalue
program define modlpr, eclass

	syntax varlist(ts max=1) [if] [in] [ , Powers(numlist >0 <1) noTrend]  

	if "`powers'" == "" { local powers=0.5 }
   	marksample touse
	...
	di in gr _n "Modified LPR estimate of fractional differencing parameter"
	di in gr _dup(78) "-"
...
*		capture noisily LPREst1 ``i'' `varlist' `touse'
	...
	mat `lpr' = `lpr''
	estimates clear
	estimates scalar N_powers = `N_power'
	estimates local depvar `varlist'
	estimates local power `power'
	estimates matrix modlpr `lpr'
end

program define LPREst1, rclass
	args power varlist touse
	tempname xr xi n lpg re im tcos tsin ahat bhat lpgcorr lsin matt vcv var
			/* generate fft */
	quietly {
