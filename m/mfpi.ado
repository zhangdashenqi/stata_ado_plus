...
syntax [if] [in] [aw fw pw iw] [, ///
  SELect(string) ADDpowers(string) ADJust(varlist fv) ADJBin all ALpha(string) ATx(varlist fv) ///
  CEnter(passthru) CENTer(varlist) DEAD(varname) DETail DF(string) FLex(int 1) ///
  FP1(varlist) FP2(varlist) GENF(string) GENDiff(string) LINear(string) OUTcome(string) ///
  POWers(string) noSCAling noCI SHOwmodel TReatment(varname fv) WITH(varname fv) ///
  ZERo(string) MFPopts(string) ]

// Process mfpi options
if "`atx'" != "" & "`genf'`gendiff'" == "" {
	// out of sample prediction at `atx' only with functions or treatment effect function
	di as err "genf() and/or gendiff() required with atx()"
	exit 198
}
if "`with'" != "" {
	if "`treatment'" != "" {
...
if "`with'" == "" {
	di as err "option treatment() required"
	exit 198
}
fvexpand `with'
if "`r(fvops)'" == "" {
	...
	while "`1'"!="" {
		if "`1'"=="`lsep'" {
			...
			if `lparen'==0 {
				...
				forvalues i = 1 / `none' {
					local thing : word `i' of `one'
					local ++ncat
					fvunab iz`ncat': `thing'
					fvexpand `iz`ncat''
					local isfactor`ncat' = ("`r(fvops)'" == "true")
					local linear `linear' `iz`ncat''
					local ifp`ncat' 0
...
local Xvars: list xvars - With
quietly {
	levelsof `With' `ifuse', local(levels)
	...
	forvalues ni = 1 / `items' {
		 // out of sample prediction at var `atx'
		local z `iz`ni''
...
*/
		if `degree' > 0 {
			...
			// set up variable to predict at. Applies only to final item in list.
			if "`atx'" != "" {
				local atxni : word `ni' of `atx'
				if "`atxni'" == "" {
					noi di as err "no out-of-sample prediction variable found for vn(`ni')"
					exit 198
				}
				local atiz _`atxni'
				cap drop `atiz'
				gen `atiz' = (`atxni'+`shift')/`scale'
				// if `atiz' is non-positive, set it to missing
				if (`atiz' <= 0) replace `atiz' = .
			}
/*
	Main-effects model.
...
local adjvl `xvars' `adjust'
local interactvl `with'##c.(`vi')
		local devint = -2*e(ll)
			...
			if `degree' > 0 {
				...
				forvalues i = 1 / `nlevels' {
					if `flex' <= 3 {
						if `i' == 1 {
							if "`atx'" != "" {
								fracgen `atiz' `powint', replace noscaling `center' name(Iz`i')
							}
							else fracgen `iz' `powint' `ifuse', replace noscaling `restrict' `center' name(Iz`i')
							local Izdrop `r(names)'
						}
					}
					else {
						if "`atx'" != "" {
							fracgen `atiz' `powint`i'', replace noscaling `center' name(Iz`i')
						}
						else fracgen `iz' `powint`i'' `ifuse', replace noscaling `restrict' `center' name(Iz`i')
				...
				if `isfactor`ni'' { // linear term is a factor
					...
					levelsof `Z' `ifuse', local(Zlevels)
					local nZ : word count `Zlevels'
					if "`atx'" != "" {
						// note: not checking if levels of `atx' correspond to levels of `z'
						local Zoos = substr("`atx'", 1 + strpos("`atx'", "."), .)
					}
					forvalues j = 1 / `nZ' {
						local lev : word `j' of `Zlevels'
						forvalues i = 1 / `nlevels' {
							if "`atx'" != "" {
								local FP`i'`j' `lev'.`Zoos'
							}
							else local FP`i'`j' `lev'.`Z'
							if (`i' == 1) {
								local prefix`j' `lev'
								local vi`j' `Z'
							}
					...
					forvalues i = 1 / `nlevels' {
						forvalues j = 1 / `nz' {
							if "`atx'" != "" {
								local FP`i'`j' : word `j' of `atx'
							}
							else local FP`i'`j' : word `j' of `z'
							if (`i' == 1) {
								local vi`j' :  word `j' of `z'
								local prefix`j' c
							}
						}
					}
...
return local adjvl `adjvl'
return local interactvl `interactvl'
end
