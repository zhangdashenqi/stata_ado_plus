...
{synopt :{opt at:x(varlist)}}predict fitted functions or difference between functions at x-variables in {it:varlist}{p_end}
...
{synopt :{opt fp1(fp1_zvarlist)}}use fractional polynomials (FPs) of degree 1 for main effects and interactions with each member of {it:fp1_zvarlist}{p_end}
{synopt :{opt fp2(fp2_zvarlist)}}use FPs of degree 2 for main effects and interactions with each member of  {it:fp2_zvarlist}{p_end}
...
{synopt :{opt lin:ear(linear_zvarlist)}}use linear functions for main effects and interactions with each member of  {it:linear_zvarlist}{p_end}
...
{phang}
{cmd:atx(}{it:varlist}{cmd:)} predicts functions or their differences
out-of-sample at the covariate values in {it:varlist}.
This option is valid only with the options {opt genf()} or
{opt gendiff()}. The variables in {it:varlist} should correspond 1:1
with those whose interactions are to be analysed, as described under
{opt vn(#)} in {cmd:mfpi_plot}.
...
{phang}{cmd:. range ER 0 1000 100}{p_end}
{phang}{cmd:. range PGR 0 500 80}{p_end}
{phang}{cmd:. mfpi, treatment(hormon) fp1(er pgr) flex(3) gendiff(d) atx(ER PGR): stcox age enodes}{p_end}
{phang}{cmd:. mfpi_plot ER}{p_end}
{phang}{cmd:. mfpi_plot PGR, vn(2)}
...
{phang}{cmd:. mkspline age_ = age, cubic nknots(4)}{p_end}
{phang}{cmd:. mfpi, select(0.05) treatment(i.treat) linear((age_1 age_2 age_3)) gendiff(d): stcox z1 z2 z3}{p_end}
{phang}{cmd:. mfpi_plot age}
...
{phang}Royston, P., and W. Sauerbrei. 2013. Interaction of treatment with a
continuous variable: simulation study of significance level for several methods
of analysis. {it:Statistics in Medicine} DOI: 10.1002/sim.5813.
Print version to appear in September 2013.
