...
if "`cmd'"=="margprev" {;
  ...
  * Create new estimation results *;
  syntax [if] [in] [pweight aweight fweight iweight], [ ATspec(string asis)
    `options' subpop(passthru) PRedict(passthru) vce(passthru) noEsample force ITERate(passthru)
    ];
  *
  ...
  *;
  ...
  marksample touse;
  qui margins if `touse' [`weight'`exp'], at(`atspec') `subpop' `predict' `vce' `esample' `force' post;
  * Collect scalar e-results from margins *;
  ...
  qui nlcom ("Scenario_1":logit(_b[_cons])), `iterate' post;
