...
{synopt :{opt nor:obust}}displays results based on the non-robust covariance matrix estimator{p_end}
...
and Powell (1984). Standard errors that are also robust to intra-cluster correlation can be obtained
with the option {opt cluster}; see Parente and Santos Silva (2013). Non-robust results can also be displayed 
with the option {opt norobust}. Additionally, {cmd:qreg2} reports the value of the objective function, defined as 
the average of the check function; the R-squared, defined as the square of the correlation between the fitted 
values and the dependent variable (note however that in quantile regressions the R-squared is even less meaningful 
...
option {opt cluster} is used the Parente-Santos Silva (2013) test for intra-cluster correlation is reported. The 
...
allowing for intra-cluster correlation as in Parente and Santos Silva (2013); the 
variable identifying the clusters is defined by {it:clustvar}.

{phang}{opt norobust} display results based on the non-robust covariance matrix estimator.
...
Parente-Santos Silva (2013) tests.
...
official Stata command. For further help and support, please contact jmcss@essex.ac.uk. Please notice 
...
{pstd}Median regression with non-robust output{p_end}
{phang2}{cmd:. qreg2 price weight length foreign, nor}{p_end}
...
{synopt:{cmd:e(rank)}}rank of the non-robust covariance matrix{p_end}
...
{synopt:{cmd:e(cmd)}}{cmd:qreg2}{p_end}
...
{phang} Machado, J.A.F. and Santos Silva, J.M.C. (2000), Glejser's Test Revisited, {it:Journal of Econometrics}, 
97(1), 189-202.{p_end} 
{phang} Parente, P.M.D.C. and Santos Silva, J.M.C. (2013), Quantile regression with clustered data, 
{browse "http://www.essex.ac.uk/economics/discussion-papers/papers-text/dp728.pdf" : Department of Economics, University of Essex, Discussion Paper No 728}.{p_end} 
