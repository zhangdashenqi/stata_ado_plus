...
*---------------------------------------------------------------
	...
	if ("`fam3'" == "gau" | "`fam3'" == "nor" | "`fam1'" == "") {
		...
		***** in case nodisplay covers non-convergence 
...
*---------------------------------------------------------------
	...
	if ("`fam2'" == "po" ) { 
		...
		***** in case nodisplay covers non-convergence 	
		...
		qui predict double `mu1', mu
		qui gen double `q' = `dep' * log(`mu1') - `mu1' - `dep' * (log(`dep') - 1)
...
*---------------------------------------------------------------
	...
	if ("`fam2'" == "nb" ) { 
		...
		***** in case nodisplay covers non-convergence 	
		...
		qui predict double `mu1', mu
		local k = 1 / `nbpara'
		qui gen double `q' = - ln(1 + `k' * `mu1') / `k' if `dep' == 0
		replace `q' = - `dep' * ln(`dep' / `mu1') + (1 + `k' * `dep') / `k' 	/*
			*/	* ln((1 + `k' * `dep') / (1 + `k' * `mu1')) if `dep' ~= 0
		sum `q', meanonly
		scalar qic = -2 * r(sum) + 2 * off
		scalar qicu = -2 * r(sum) + 2 * par
