...
# Code to perform randomization inference in window
...
program define rdrandinf, rclass
	
	syntax varlist(min=2 max=2 numeric) [if] [in] [, Cutoff(real 0) wl(numlist max=1) wr(numlist max=1) /*
		*/ reps(integer 1000) STATistic(string) p(integer 0) NULLtau(real 0) evall(numlist max=1) evalr(numlist max=1) /*
		*/ kernel(string) ci(string) INTERFci(real 0) seed(integer 666) fuzzy(namelist min=1 max=2) d(numlist max=1) dscale(numlist max=1) /*
		*/ BErnoulli(string) COVariates(namelist) obsmin(numlist max=1) obsstep(numlist max=1) /*
		*/ wmin(numlist max=1) wstep(numlist max=1) NWindows(real 10) rdwstat(string) /*
		*/ APPROXimate rdwreps(real 1000) level(real .15) /*
		*/ plot graph_options(string) QUIetly]
	...
	if `interfci'<0|`interfci'>1{
		...
		if "`statistic'"!=""&"`statistic'"!="ttest"&"`statistic'"!="ksmirnov"&"`statistic'"!="ranksum"{
			di as error "only ttest, ksmirnov or ranksum allowed when interfci is specified"
			exit 198
		}
	}
...
*** Window
	...
	if "`wl'"!="" & "`wr'"!=""{
		...
		if "`covariates'"!=""{
			...
			if "`quietly'"==""{
				rdwinselect `r' `covariates' `if' `in', c(`cutoff') `obsmin_opt' `obsstep_opt' `wmin_opt' `wstep_opt' /*
					*/ `nwindows_opt' `rdwstat_opt' `approximate_opt' reps(`rdwreps') `level_opt' `plot_opt' `graph_opt'
			}
			else {
				qui rdwinselect `r' `covariates' `if' `in', c(`cutoff') `obsmin_opt' `obsstep_opt' `wmin_opt' `wstep_opt' /*
					*/ `nwindows_opt' `approximate_opt' reps(`rdwreps') `level_opt' `plot_opt' `graph_opt'
			}	
...
*** Summary statistics
	...
	qui count
	local n_tot=r(N)
	qui count if `tr'==1 `ifcond' & `r'!=. & `Y'!=. `in'
	local n_tot_right=r(N)
	qui count if `tr'==0 `ifcond' & `r'!=. & `Y'!=. `in'
	local n_tot_left=r(N)
	
	qui count if `inwindow' `ifcond' `in' & `tr'==1 & `r'!=. & `Y'!=.
	local n_right=r(N)
	qui count if `inwindow' `ifcond' `in' & `tr'==0 & `r'!=. & `Y'!=.
	local n_left=r(N)
	qui sum `Y' if `inwindow' `ifcond' `in' & `tr'==1 & `r'!=. & `Y'!=.
	local m_right=r(mean)
	local s_right=r(sd)
	qui sum `Y' if `inwindow' `ifcond' `in' & `tr'==0 & `r'!=. & `Y'!=.
	local m_left=r(mean)
	local s_left=r(sd)	
...
*** Results
	...
	** Observed values, asymptotic p-values and power
	...
	qui{
		...
		if "`statistic'"=="ttest"|"`statistic'"==""|"`statistic'"=="all"{
			if `p'==0{
		...
		if "`statistic'"=="ksmirnov"|"`statistic'"=="all"{
			ksmirnov `Y_adj_null', by(`tr') exact
			if "`statistic'"=="all"{
				local obs_stat2 = r(D)
				if `p'==0{
					local asy_p2 = r(p_exact)
					local power2 = "."
				}
				else {
					local asy_p2 "."
	...
	if "`fuzzy_stat'"!="tsls"{
		...
		if "`bernoulli'"==""{
			...
			if "`statistic'"=="all"{
				...
				mata: st_numscalar("p1",mean(abs(B[.,1]):>=abs(`obs_stat1')))
				mata: st_numscalar("p2",mean(abs(B[.,2]):>=abs(`obs_stat2')))
				mata: st_numscalar("p3",mean(abs(B[.,3]):>=abs(`obs_stat3')))
				matrix aux1 = (p1,p2,p3)
				matrix aux2 = (`obs_stat1',`obs_stat2',`obs_stat3')
				...
				mata: st_numscalar("p1",mean(abs(B[.,1]):>=abs(`obs_stat')))
				local randp = p1
			}
		}
	} 
