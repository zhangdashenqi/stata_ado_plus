...
program define rdrbounds, rclass sortpreserve
	...
	syntax varlist(min=2 max=2 numeric) [if] [in] [, Cutoff(real 0) prob(string)  /*
		*/ reps(integer 500) bound(string) ulist(numlist) wlist(numlist)  /*
		*/ GAMMAlist(numlist) expgamma(numlist) FMpval  STATistic(string) p(integer 0) evalat(string) /*
		*/ kernel(string) NULLtau(real 0) seed(integer 666) fuzzy(namelist min=1 max=1) ]
	...
	else if "`statistic'"=="ksmirnov"|"`statistic'"=="ttest"{
	...
	** Randomization p-value
	...
	if "`fmpval'"==""{
		foreach w of numlist `wlist'{
			...
			if "`prob'"==""{
				qui sum `treatvar' if `runvar'>=-`w' & `runvar'<=`w' `ifcond' `in' & `outvar'!=. & `runv_aux'!=.
				qui gen `probs' = r(mean)
			}
			
			if "`evalat'"=="means"{
				qui sum `runv_aux' if `treatvar'==1 & abs(`runvar')<=`w' `ifcond' `in' & `outvar'!=. & `runv_aux'!=.
				local evalr = r(mean)
				qui sum `runv_aux' if `treatvar'==0 & abs(`runvar')<=`w' `ifcond' `in' & `outvar'!=. & `runv_aux'!=.
				local evall = r(mean)
				local eval_opt "evall(`evall') evalr(`evalr')"
			}
		
			qui rdrandinf `outvar' `runvar' `if' `in', wl(-`w') wr(`w') bernoulli(`probs') reps(`reps') /*
		...
		foreach w of numlist `wlist'{
			...
			qui rdrandinf `outvar' `runvar' `if' `in', wl(-`w') wr(`w') reps(`reps') /*
				*/ p(`p') nulltau(`nulltau') `stat_opt' `eval_opt' `kernel_opt' `fuzzy_opt'
			local pval = r(randpval)
	...
	foreach gamma of numlist `gamma_list'{
		...
		foreach w of numlist `wlist'{
			...
			if "`ulist'"==""{
				local min = 0
				local step = 1
				qui count if abs(`runvar')<=`w' `ifcond' `in'  & `outvar'!=. & `runv_aux'!=.
				local max = r(N)
			...
			if "`bound'"==""|"`bound'"=="both"{
				...
				** Search over U vectors
				...
				preserve
				qui keep if `runvar'>=-`w' & `runvar'<=`w' `ifcond' `in' & `outvar'!=. & `runv_aux'!=.
				...
				foreach nu of numlist `ulist'{
					...
					qui {					
						...
						*local gamma0 = log(`prob'/(1-`prob'))
						*gen `prob_h' = exp(`gamma0'+`gamma'*`unobs_h')/(1+exp(`gamma0'+`gamma'*`unobs_h'))
						*gen `prob_l' = exp(`gamma0'+`gamma'*`unobs_l')/(1+exp(`gamma0'+`gamma'*`unobs_l'))
						

						gen `prob_h' = `pl'*`unobs_h' + `ph'*(1-`unobs_h')
						gen `prob_l' = `pl'*`unobs_l' + `pl'*(1-`unobs_l')
...
**************************************
** Auxiliary program to display output
...
program define output_line
	...
	if "`omitg'"==""{
		...
		if "`line'"=="top"{
			...
			di as text %5.2f `gamma' _col(10) %5.2f exp(`gamma') _col(12) "{ralign 15: upper bound}" as text "{c |}" as res `numdisp'
		}
	}
