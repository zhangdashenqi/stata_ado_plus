*! radiusmatch 1.0 Andreas Steinmayr 10dec2012  
...
program define radiusmatch, rclass sortpreserve
	...
	if `bootstrap' == 0 {
		...
		mahal(`mahalanobis') score(`scoreweight') touse(`touse') `index' `logit' `nocommon' `ate' bc(`bc') `boost'
		_mktab, `ate' bc(`bc')
		return add
		...
		forvalues i = 1(1)`bootstrap' {
			...
			mahal(`mahalanobis') score(`scoreweight') touse(`touse') `index' `logit' `nocommon' `ate' bc(`bc') `boost' rep
			timer on 4
		...
		preserve
		use "`bfile'", clear
		mata : est_tstat(`colnum')
		
		// create output tables
		_mk_bstab `outcome', effect("atet") bc(`bc')
		return add
		if ("`ate'"!="") {
				_mk_bstab `outcome', effect("atent") bc(`bc')
				return add
				_mk_bstab `outcome', effect("ate") bc(`bc')
				return add
		}
...
program define matchcore, rclass sortpreserve
	...
	if ("`varlist'"!="") {
		...
		if (`e(converged)'==1) {
			qui predict double _pscore, `index'
		}
...
**************************************************************************************************************	
	...
	// generate results
	if ("`outcome'"!="") {
		_estresults `outcome', `ate' p2(`_pscore2') m(`mahalanobis') touse(`touse') `rep' bc(`bc') `knn'
	}
...
syntax varlist [, ate m(string) p2(string) touse(string) postid(string) rep bc(integer 2) knn]
...
foreach v of varlist `varlist' {
	...
	// Bias correction 
		...
		if (`bc'==2) { // linear and logit bias correction
			tempvar _yzero
			qui count if `v' ==0 & `treat'==0 & _support==1
			local nullen = r(N)
			qui count if `v' ==1 & `treat'==0 & _support==1
			local einsen = r(N)
			qui count if `treat'==0 & _support==1
...
syntax namelist [, effect(string) bc(integer 2)]
		...
		matrix bsp`effect' = p_bs[1,`i'..`stop']
		...
		foreach var in `namelist' {
				...
				local p = p_bs[1,`i']
				display as text %-16s abbrev("`var'",16) "  {c |} " as result %7.0g `coeff' "   " %7.0g `r(mean)' "   " %7.0g `r(sd)' "   " %5.0g `p' "   " %7.0g `5lo' "   " %7.0g `10lo'  "   " %7.0g `med' "   " %7.0g `10up' "   " %7.0g `5up'
