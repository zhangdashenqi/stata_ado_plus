*! version 1.2.1    05sep2000       C F Baum/V Wiggins  (STB-57: sts16)
...
program define roblpr, rclass
	...
	tempname xr xi n obs pg lpg ok vok vcv bs
	tempvar var vecy 
	...
	if `nrvars' != 1 | "`regressonly'" != "" {
		...
		while `i'<=`nrvars' {
			...
			if `j'==1 {
				qui gen double `lpg' = log((`xr'^2+`xi'^2)) /*
					*/ if `touse'
				} 
			else 	{
			...
			qui replace `vecy'=`lpg'[_n-`offset'] 		/*
				*/ in `first'/`last'
			if `j'>1 {
				qui replace `ok'=`vok'[_n-`offset'] 	/*
					*/ in `first'/`last'
				drop `vok'
				}
			tempvar llam iota
			...
			/* log periodogram regression: use noc to get 
			 * indiv constant terms */
		if "`constraints'" != "" {
			cons def 988 _cons = 0
			qui cnsreg `xlist' if `ok'>0 , 	/*
				*/ constraint(`constraints' 988)
		}
		else	qui regress `xlist' if `ok'>0, noc
	...
	if `nrvars'==1 {
		di in gr _n "Robinson estimates of fractional differencing parameter"
		if `l' != 0 {
		...
		if "`l'" != "" { local optbase l(`l') }
		if "`j'" != "" { local optbase `optbase' j(`j') }
		tokenize `powers'
		local i 1
		while "``i''" != "" {
			capture roblpr `varlist' if `touse',	/*
				*/ regressonly `optbase' power(``i'')
			if _rc {
				di in blue "  roblpr could not be " 	/*
					*/ " calculated for power = ``i''"
			}
			else {
