*! 2.2.0 NJC 30 March 2009 
* 2.1.1 NJC 5 October 2007 
...
program regplot
	...
	BY(str asis) YTItle(str asis) MSymbol(str) SOrt recast(str)         ///
	plot(str asis) addplot(str asis) * ] 

	// initial checking and picking up what -regress- type command 
	// leaves behind

	// plottype not documented 2.2.0 but not treated as error 
	if "`recast'" != "" & "`plottype'" != "" { 
		di as txt "recast() trumps plottype()" 
	}
	else if "`plottype'" != "" { 
		local recast `plottype' 
	} 
