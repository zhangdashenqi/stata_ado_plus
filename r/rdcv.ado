...
foreach T in `nsamp' {
	...
	* for discrete data: omit 5 largest and smallest values
	qui su `indepvar'
			...
			if `witer'==1 {
				...
				*di "no. of obs is " _N
				preserve
					...
					tempname rot_at
					mkmat `indepvar', mat(`rot_at')
					mat coln `rot_at'=c1
				restore
				svmat `rot_at'
				if "`samebwidth'"=="" {
				lpoly `depvar' `indepvar' if `group`T'', nogr deg(`degree') ///
					kernel(`kernname') at(`rot_at'1)
				}
				else {
...
foreach T in `nsamp' {
	...
	*else: default: at-variable corresponds to all unique values of indepvar
		...
		preserve
			keep `indepvar'
			qui duplicates drop `indepvar', force
			mkmat `indepvar', mat(atpoints)
			mat coln atpoints=c1
		restore
		svmat atpoints
		qui g iat`T'=atpoints1 
		if "`threshold'"!="" {
