*! version 1.1.5  1oct03 arb
*! Reformats the output from regression commands
*! (c) Sealed Envelope Ltd, 2002-03
			...
			if substr("`term'",1,length("`prefix'"))=="`prefix'" {
				...
				* Baseline group for this term (except interactions)
				if missing("`inter'") {
					...
					if missing("`basel'") {
						if !missing("`prev'") {
							* baseline is most prevalent group
							preserve
							tempvar catfreq
							drop if missing(`varnme1')
							sort `varnme1'
							by `varnme1': gen int `catfreq'=_N
							sort `catfreq'
							local basel = `varnme1'[_N]
							restore
						}
						else if "`type1'"=="num" {
